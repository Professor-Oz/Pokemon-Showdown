...
let BattleAbilities = {
	...
	battery: {
		...
		onAllyBasePower(basePower, attacker, defender, move) {
			if (
				attacker !== this.effectData.target &&
	...
	gulpmissile: {
		...
		onAfterMove(pokemon, target, move) {
			if (
				pokemon.template.species !== "Cramorant" ||
				pokemon.transformed ||
				!["dive", "surf"].includes(move.id) ||
				pokemon.volatiles["dive"]
			)
				return;
			const forme =
	...
	iceface: {
		...
		onDamage(damage, target, source, effect) {
			if (
				...
				target.template.speciesid === "eiscue" &&
				!target.transformed
			) {
		...
		onUpdate(pokemon) {
			if (
				pokemon.template.speciesid === "eiscue" &&
				this.effectData.busted
			) {
			...
			for (const target of pokemon.side.foe.active) {
				...
				if (target.volatiles["substitute"]) {
					...
					this.add(
						"-immune",
						target,
						`[from] ability: ${this.dex.getAbility(target.ability).name}`
					);
				} else {
	...
	propellertail: {
		...
		onRedirectTarget(target, source, source2, move) {
			...
			this.debug(
				`onRedirectTarget: ${target} (${target.side.name}), ${source} (${source.side.name}), ${source2}, ${move}`
			);
