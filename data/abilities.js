...
-1: Detrimental
 ...
 1: Ineffective
          An ability that has minimal effect or is only useful in niche
situations. ex. Light Metal, Suction Cups
 ...
 4: Very useful
          One of the more popular abilities. It requires minimal support to be
effective. ex. Adaptability, Magic Bounce
...
let BattleAbilities =
    {
      ...
      "airlock" : {
        ...
        onStart(pokemon) { this.add('-ability', pokemon, 'Air Lock'); },
      ...
      "angerpoint" : {
        ...
        onHit(target, source, move) {
          ...
          if (move && move.effectType === 'Move' &&
            ...
            this.add('-setboost', target, 'atk', 12,
      ...
      "arenatrap" : {
        ...
        onFoeMaybeTrapPokemon(pokemon, source) {
          ...
          if (!source || !this.isAdjacent(pokemon, source))
            return;
          if (pokemon.isGrounded(!pokemon.knownType)) { // Negate immunity if
                                                        // the type is unknown
        ...
        onAllyTryAddVolatile(status, target, source, effect) {
          if ([
            ...
            if (effect.effectType === 'Move') {
              this.add('-activate', this.effectData.target,
                       'ability: Aroma Veil', '[of] ' + target);
            }
            return null;
          }
        },
      ...
      "aurabreak" : {
        ...
        onStart(pokemon) { this.add('-ability', pokemon, 'Aura Break'); },
      ...
      "battlebond" : {
        ...
        onSourceFaint(target, source, effect) {
          if (effect && effect.effectType === 'Move' &&
              source.template.speciesid === 'greninja' && source.hp &&
              !source.transformed && source.side.foe.pokemonLeft) {
      ...
      "berserk" : {
        ...
        onAfterMoveSecondary(target, source, move) {
          ...
          if (!lastAttackedBy)
            return;
          const damage =
              move.multihit ? move.totalDamage : lastAttackedBy.damage;
        ...
        onBoost(boost, target, source, effect) {
          ...
          if (boost.def && boost.def < 0) {
            ...
            if (!(/** @type {ActiveMove} */ (effect)).secondaries) {
              this.add("-fail", target, "unboost", "Defense",
      ...
      "cloudnine" : {
        shortDesc :
            "While this Pokemon is active, the effects of weather conditions are disabled.",
        onStart(pokemon) { this.add('-ability', pokemon, 'Cloud Nine'); },
        suppressWeather : true,
        id : "cloudnine",
        name : "Cloud Nine",
        rating : 2,
      ...
      "comatose" : {
        ...
        // Permanent sleep "status" implemented in the relevant sleep-checking
        // effects
        isUnbreakable : true,
        id : "comatose",
        name : "Comatose",
      ...
      "cottondown" : {
        ...
        shortDesc :
            "Lowers Speed of all Pokémon except itself when hit by an attack.",
      ...
      "damp" : {
        ...
        onAnyTryMove(target, source, effect) {
          if ([ 'explosion', 'mindblown', 'selfdestruct' ].includes(
                  effect.id)) {
            this.attrLastMove('[still]');
            this.add('cant', this.effectData.target, 'ability: Damp', effect,
                     '[of] ' + target);
            return false;
          }
        },
      ...
      "darkaura" : {
        ...
        onStart(pokemon) { this.add('-ability', pokemon, 'Dark Aura'); },
        onAnyBasePower(basePower, source, target, move) {
          ...
          return this.chainModify(
              [ move.hasAuraBreak ? 0x0C00 : 0x1547, 0x1000 ]);
        },
        isUnbreakable : true,
        id : "darkaura",
        name : "Dark Aura",
      ...
      "dauntlessshield" : {
        ...
        onStart(pokemon) { this.boost({def : 1}, pokemon); },
        id : "dauntlessshield",
      ...
      "dazzling" : {
        ...
        onFoeTryMove(target, source, effect) {
          if ((source.side === this.effectData.target.side ||
              ...
              effect.priority > 0.1 && effect.target !== 'foeSide') {
            this.attrLastMove('[still]');
            this.add('cant', this.effectData.target, 'ability: Dazzling',
                     effect, '[of] ' + target);
            return false;
          }
        },
      ...
      "deltastream" : {
        ...
        onAnySetWeather(target, source, weather) {
          if (this.field.getWeather().id === 'deltastream' &&
              !['desolateland', 'primordialsea', 'deltastream'].includes(
                  weather.id))
            return false;
        },
        onEnd(pokemon) {
      ...
      "desolateland" : {
        ...
        onStart(source) { this.field.setWeather('desolateland'); },
        onAnySetWeather(target, source, weather) {
          if (this.field.getWeather().id === 'desolateland' &&
              !['desolateland', 'primordialsea', 'deltastream'].includes(
                  weather.id))
            return false;
        },
      ...
      "disguise" : {
        ...
        onDamage(damage, target, source, effect) {
          if (effect && effect.effectType === 'Move' &&
              [ 'mimikyu', 'mimikyutotem' ].includes(
                  target.template.speciesid) &&
              !target.transformed) {
        ...
        onUpdate(pokemon) {
          if ([ 'mimikyu', 'mimikyutotem' ].includes(
                  pokemon.template.speciesid) &&
              this.effectData.busted) {
            let templateid = pokemon.template.speciesid === 'mimikyutotem'
      ...
      "drizzle" : {
        ...
        onStart(source) {
          for (const action of this.queue) {
            if (action.choice === 'runPrimal' && action.pokemon === source &&
      ...
      "fairyaura" : {
        ...
        onStart(pokemon) { this.add('-ability', pokemon, 'Fairy Aura'); },
        onAnyBasePower(basePower, source, target, move) {
          if (target === source || move.category === 'Status' ||
              move.type !== 'Fairy')
            return;
      ...
      "flashfire" : {
        ...
        effect : {
          ...
          onStart(
              target) { this.add('-start', target, 'ability: Flash Fire'); },
          onModifyAtkPriority : 5,
          onModifyAtk(atk, attacker, defender, move) {
            if (move.type === 'Fire') {
      ...
      "flowergift" : {
        ...
        onStart(pokemon) { delete this.effectData.forme; },
        onUpdate(pokemon) {
          if (!pokemon.isActive ||
          ...
          if ([ 'sunnyday', 'desolateland' ].includes(
            ...
            if (pokemon.template.speciesid !== 'cherrimsunshine') {
              pokemon.formeChange('Cherrim-Sunshine', this.effect, false,
                                  '[msg]');
            }
          } else {
            if (pokemon.template.speciesid === 'cherrimsunshine') {
      ...
      "flowerveil" : {
        ...
        onAllyBoost(boost, target, source, effect) {
          ...
          if (showMsg && !(/** @type {ActiveMove} */ (effect)).secondaries) {
            this.add('-fail', this.effectData.target, 'unboost',
                     '[from] ability: Flower Veil', '[of] ' + target);
          }
        },
        onAllySetStatus(status, target, source, effect) {
          if (target.hasType('Grass') && source && target !== source &&
              effect && effect.id !== 'yawn') {
            this.debug('interrupting setStatus with Flower Veil');
            if (effect.id === 'synchronize' ||
                (effect.effectType === 'Move' && !effect.secondaries)) {
              this.add('-activate', this.effectData.target,
                       'ability: Flower Veil', '[of] ' + target);
            }
            return null;
          }
        },
      ...
      "forecast" : {
        ...
        onUpdate(pokemon) {
          if (pokemon.baseTemplate.baseSpecies !== 'Castform' ||
              pokemon.transformed)
            return;
          let forme = null;
        ...
        onStart(pokemon) {
          ...
          for (const target of pokemon.side.foe.active) {
            ...
            for (const moveSlot of target.moveSlots) {
              ...
              if (move.ohko)
                bp = 150;
              if (move.id === 'counter' || move.id === 'metalburst' ||
                  move.id === 'mirrorcoat')
                bp = 120;
              if (bp === 1)
                bp = 80;
        ...
        onBoost(boost, target, source, effect) {
          ...
          if (showMsg && !(/** @type {ActiveMove} */ (effect)).secondaries) {
            this.add("-fail", target, "unboost",
                     "[from] ability: Full Metal Body", "[of] " + target);
          }
        },
        isUnbreakable : true,
        id : "fullmetalbody",
      ...
      "gorillatactics" : {
        ...
        onStart(pokemon) { pokemon.abilityData.choiceLock = ""; },
        onBeforeMove(pokemon, target, move) {
          if (move.isZPowered || move.maxPowered || move.id === 'struggle')
            return;
          if (pokemon.abilityData.choiceLock &&
              pokemon.abilityData.choiceLock !== move.id) {
            // Fails unless ability is being ignored (these events will not
            // run), no PP lost.
        ...
        onEnd(pokemon) { pokemon.abilityData.choiceLock = ""; },
        id : "gorillatactics",
      ...
      "grassysurge" : {
        shortDesc :
            "On switch-in, this Pokemon summons Grassy Terrain.",
      ...
      "gulpmissile" : {
        desc :
            "When the Pokémon uses Surf or Dive, it will come back with prey. When it takes damage, it will spit out the prey to deal 25% damage. If the base HP is below 50%, the prey will be a Pikachu and paralyze the opponent after being damaged. Otherwise, the prey is an Arrokuda and will lower the opponent's Def by 1 stage after being damaged.",
        shortDesc :
            "Get prey with Surf/Dive. When taking damage, prey is used to attack.",
        onDamagePriority : -1,
        onDamage(damage, target, source, effect) {
          ...
          if (effect && effect.effectType === 'Move' &&
              [ 'cramorantgulping', 'cramorantgorging' ].includes(
                  target.template.speciesid) &&
              !target.transformed) {
            // Forme change before damaging to avoid a potential infinite loop
            // with surf cramorant vs surf cramorant
        ...
        onAfterMove(pokemon, target, move) {
          if (pokemon.template.species !== 'Cramorant' || pokemon.transformed ||
              !['dive', 'surf'].includes(move.id) || pokemon.volatiles['dive'])
            return;
          const forme = pokemon.hp <= pokemon.maxhp / 2 ? 'cramorantgorging'
                                                        : 'cramorantgulping';
      ...
      "healer" : {
        ...
        onResidual(pokemon) {
          ...
          for (const allyActive of pokemon.side.active) {
            if (allyActive && allyActive.hp &&
                this.isAdjacent(pokemon, allyActive) && allyActive.status &&
      ...
      "hungerswitch" : {
        ...
        shortDesc :
            "Changes between Full Belly and Hangry Mode at the end of each turn.",
        onResidual(pokemon) {
          if (pokemon.template.baseSpecies !== 'Morpeko' || pokemon.transformed)
            return;
          let targetForme = pokemon.template.species === 'Morpeko'
                                ? 'Morpeko-Hangry'
                                : 'Morpeko';
      ...
      "hustle" : {
        ...
        // This should be applied directly to the stat as opposed to chaining
        // with the others
        onModifyAtkPriority : 5,
      ...
      "hydration" : {
        ...
        onResidualSubOrder : 1,
        onResidual(pokemon) {
          if (pokemon.status && [ 'raindance', 'primordialsea' ].includes(
                                    pokemon.effectiveWeather())) {
      ...
      "hypercutter" : {
        ...
        onBoost(boost, target, source, effect) {
          ...
          if (boost.atk && boost.atk < 0) {
            delete boost.atk;
            if (!(/** @type {ActiveMove} */ (effect)).secondaries) {
              this.add("-fail", target, "unboost", "Attack",
      ...
      "iceface" : {
        ...
        shortDesc :
            "(Eiscue only) First physical hit deals 0 damage, breaks ice head.",
        onDamagePriority : 1,
        onStart(pokemon) {
          if (this.field.isWeather('hail') &&
              pokemon.template.speciesid === 'eiscuenoice' &&
              !pokemon.transformed) {
        ...
        onDamage(damage, target, source, effect) {
          if (effect && effect.effectType === 'Move' &&
              ...
              target.template.speciesid === 'eiscue' && !target.transformed) {
      ...
      "icescales" : {
        shortDesc :
            "This Pokémon's Special Defense is doubled.",
        // TODO verify this is the correct way to implement this
        onModifySpD(spd) { return this.chainModify(2); },
        id : "icescales",
        name : "Ice Scales",
        rating : 3.5,
        num : 246,
      ...
      "illusion" : {
        ...
        onAfterDamage(damage, target, source, effect) {
          if (target.illusion && effect && effect.effectType === 'Move' &&
              effect.id !== 'confused') {
            this.singleEvent('End', this.dex.getAbility('Illusion'),
                             target.abilityData, target, source, effect);
          }
        },
        onEnd(pokemon) {
          if (pokemon.illusion) {
            ...
            pokemon.illusion = null;
            let details = pokemon.template.species +
      ...
      "imposter" : {
        ...
        onStart(pokemon) {
          if (this.activeMove && this.activeMove.id === 'skillswap')
            return;
          let target = pokemon.side.foe.active[pokemon.side.foe.active.length -
                                               1 - pokemon.position];
          if (target) {
      ...
      "innardsout" : {
        ...
        onAfterDamage(damage, target, source, move) {
          if (source && source !== target && move &&
              move.effectType === 'Move' && !target.hp) {
      ...
      "keeneye" : {
        ...
        onBoost(boost, target, source, effect) {
          ...
          if (boost.accuracy && boost.accuracy < 0) {
            ...
            if (!(/** @type {ActiveMove} */ (effect)).secondaries) {
              this.add("-fail", target, "unboost", "accuracy",
      ...
      "leafguard" : {
        ...
        onTryAddVolatile(status, target) {
          if (status.id === 'yawn' && [ 'sunnyday', 'desolateland' ].includes(
                                          target.effectiveWeather())) {
            this.add('-immune', target, '[from] ability: Leaf Guard');
            return null;
          }
        ...
        onSourceHit(target, source, move) {
          ...
          if (target !== source && move.category !== 'Status') {
            ...
            if (!source.setItem(yourItem)) {
              target.item = yourItem.id; // bypass setItem so we don't break
                                         // choicelock or anything
              return;
            }
      ...
      "magnetpull" : {
        ...
        shortDesc :
            "Prevents adjacent Steel-type foes from choosing to switch.",
        onFoeTrapPokemon(pokemon) {
      ...
      "mimicry" : {
        ...
        onUpdate(pokemon) {
          // PLACEHOLDERS. What types are used for each terrain? Does the type
          // revert after?
      ...
      "mummy" : {
        ...
        onAfterDamage(damage, target, source, move) {
          if (source && source !== target && move && move.flags['contact'] &&
            ...
            if (oldAbility) {
              this.add('-activate', target, 'ability: Mummy',
                       this.dex.getAbility(oldAbility).name, '[of] ' + source);
            }
          }
        },
        onBasePower(basePower, pokemon, target, move) {
      ...
      "naturalcure" : {
        ...
        onCheckShow(pokemon) {
          // This is complicated
          // For the most part, in-game, it's obvious whether or not Natural
          // Cure activated, since you can see how many of your opponent's
          // pokemon are statused. The only ambiguous situation happens in
          // Doubles/Triples, where multiple pokemon that could have Natural
          // Cure switch out, but only some of them get cured.
          if (pokemon.side.active.length === 1)
            return;
          ...
          for (const curPoke of pokemon.side.active) {
            ...
            if (!curPoke || !curPoke.status) {
              // this.add('-message', "" + curPoke + " skipped: not statused or
              // doesn't exist");
              continue;
            }
            if (curPoke.showCure) {
              // this.add('-message', "" + curPoke + " skipped: Natural Cure
              // already known");
              continue;
            }
            ...
            if (Object.values(template.abilities).indexOf('Natural Cure') < 0) {
              // this.add('-message', "" + curPoke + " skipped: no Natural
              // Cure");
              continue;
            }
            // pokemon's ability is known to be Natural Cure
            if (!template.abilities['1'] && !template.abilities['H']) {
              // this.add('-message', "" + curPoke + " skipped: only one
              // ability");
              continue;
            }
            ...
            if (curPoke.hasAbility('naturalcure')) {
              // this.add('-message', "" + curPoke + " confirmed: could be
              // Natural Cure (and is)");
              cureList.push(curPoke);
            } else {
              // this.add('-message', "" + curPoke + " confirmed: could be
              // Natural Cure (but isn't)");
              noCureCount++;
            }
          }
          ...
          if (!cureList.length || !noCureCount) {
            ...
            // Unlike a -hint, this is real information that battlers need, so
            // we use a -message
            this.add('-message', "(" + cureList.length + " of " +
                                     pokemon.side.name + "'s pokemon " +
        ...
        onSwitchOut(pokemon) {
          ...
          // if pokemon.showCure is undefined, it was skipped because its
          // ability is known
          ...
          // (once you know a Pokemon has Natural Cure, its cures are always
          // known)
          if (!pokemon.showCure)
      ...
      "neutralizinggas" : {
        ...
        // Ability suppression implemented in
        // sim/pokemon.ts:Pokemon#ignoringAbility
        // TODO Will abilities that already started start again? (Intimidate
        // seems like a good test case)
        onPreStart(
            pokemon) { this.add('-ability', pokemon, 'Neutralizing Gas'); },
        onEnd(source) {
          // FIXME this happens before the pokemon switches out, should be the
          // opposite order. Not an easy fix since we cant use a supported
          // event. Would need some kind of special event that gathers events to
          // run after the switch and then runs them when the ability is no
          // longer accessible. (If your tackling this, do note extreme weathers
          // have the same issue)

          // Mark this pokemon's ability as ending so Pokemon#ignoringAbility
          // skips it
          source.abilityData.ending = "true";
          for (const pokemon of this.getAllActive()) {
            if (pokemon !== source) {
              // Will be suppressed by Pokemon#ignoringAbility if needed
              this.singleEvent('Start', this.dex.getAbility(pokemon.ability),
      ...
      "normalize" : {
        ...
        onModifyType(move, pokemon) {
          if (!(move.isZ && move.category !== 'Status') &&
              !['hiddenpower', 'judgment', 'multiattack', 'naturalgift',
                'revelationdance', 'struggle', 'technoblast', 'weatherball']
                   .includes(move.id)) {
            move.type = 'Normal';
      ...
      "oblivious" : {
        ...
        onTryHit(pokemon, target, move) {
          if (move.id === 'attract' || move.id === 'captivate' ||
              move.id === 'taunt') {
      ...
      "owntempo" : {
        ...
        onHit(target, source, move) {
          if (move && move.volatileStatus === 'confusion') {
            this.add('-immune', target, 'confusion',
      ...
      "parentalbond" : {
        ...
        onPrepareHit(source, target, move) {
          ...
          if (move.category !== 'Status' && !move.selfdestruct &&
              !move.multihit && !move.flags['charge'] && !move.spreadHit &&
              !move.isZ) {
            move.multihit = 2;
        ...
        onSourceModifySecondaries(secondaries, target, source, move) {
          if (move.multihitType === 'parentalbond' &&
              move.id === 'secretpower' && move.hit < 2) {
            // hack to prevent accidentally suppressing King's Rock/Razor Fang
            return secondaries.filter(effect =>
                                          effect.volatileStatus === 'flinch');
          }
        },
        id : "parentalbond",
        name : "Parental Bond",
      ...
      "perishbody" : {
        ...
        shortDesc :
            "When hit by a contact move, the Pokémon and the attacker faint in 3 turns.",
        onAfterDamage(damage, target, source, move) {
          if (!move.flags['contact'])
            return;
      ...
      "pickup" : {
        ...
        onResidual(pokemon) {
          ...
          let pickupTargets = [];
          for (const target of this.getAllActive()) {
            if (target.lastItem && target.usedItemThisTurn &&
      ...
      "powerconstruct" : {
        ...
        onResidualOrder : 27,
        onResidual(pokemon) {
          if (pokemon.baseTemplate.baseSpecies !== 'Zygarde' ||
              pokemon.transformed || !pokemon.hp)
            return;
          if (pokemon.template.speciesid === 'zygardecomplete' ||
              pokemon.hp > pokemon.maxhp / 2)
            return;
          this.add('-activate', pokemon, 'ability: Power Construct');
          pokemon.formeChange('Zygarde-Complete', this.effect, true);
          let newHP = Math.floor(
              Math.floor(2 * pokemon.template.baseStats['hp'] +
                         pokemon.set.ivs['hp'] +
      ...
      "powerofalchemy" : {
        ...
        onAllyFaint(target) {
          ...
          let bannedAbilities = [
            'battlebond',     'comatose',     'disguise',     'flowergift',
            'forecast',       'gulpmissile',  'hungerswitch', 'iceface',
            'illusion',       'imposter',     'multitype',    'powerconstruct',
            'powerofalchemy', 'receiver',     'rkssystem',    'schooling',
            'shieldsdown',    'stancechange', 'trace',        'wonderguard',
            'zenmode'
          ];
          if (bannedAbilities.includes(target.ability))
            return;
          this.add('-ability', this.effectData.target, ability,
                   '[from] ability: Power of Alchemy', '[of] ' + target);
      ...
      "propellertail" : {
        shortDesc :
            "Ignores the effects of opposing Pokémon's moves/Abilities that redirect move targets.",
        ...
        onRedirectTarget(target, source, source2, move) {
          ...
          this.debug(`onRedirectTarget: ${target} (${target.side.name}), ${
              source} (${source.side.name}), ${source2}, ${move}`);
      ...
      "queenlymajesty" : {
        ...
        onFoeTryMove(target, source, effect) {
          if ((source.side === this.effectData.target.side ||
              ...
              effect.priority > 0.1 && effect.target !== 'foeSide') {
            this.attrLastMove('[still]');
            this.add('cant', this.effectData.target, 'ability: Queenly Majesty',
                     effect, '[of] ' + target);
            return false;
          }
        },
      ...
      "rattled" : {
        ...
        onAfterDamage(damage, target, source, effect) {
          if (effect && (effect.type === 'Dark' || effect.type === 'Bug' ||
                         effect.type === 'Ghost')) {
            this.boost({spe : 1});
          }
        },
      ...
      "receiver" : {
        ...
        onAllyFaint(target) {
          ...
          if (bannedAbilities.includes(target.ability))
            return;
          this.add('-ability', this.effectData.target, ability,
                   '[from] ability: Receiver', '[of] ' + target);
          this.effectData.target.setAbility(ability);
        },
        id : "receiver",
        name : "Receiver",
      ...
      "ripen" : {
        ...
        onSourceModifyDamage(damage, source, target, move) {
          if (target.abilityData.berryWeaken) {
            // Pokemon ate a berry that weakened damage from this attack, ripen
            // adds another 1/4 that.
            this.debug(`Ripen increases damage reduction to 3/4`);
            target.abilityData.berryWeaken = "";
            // Not sure if this is the correct multiplier to get 3/4 total,
            // assuming its taking 1/2 of 1/2 (3/4)
            return this.chainModify(0.5);
          }
        },
      ...
      "sandspit" : {
        ...
        onAfterDamage(damage, target, source, effect) {
          if (effect && effect.effectType === 'Move' &&
              effect.id !== 'confused' &&
              this.field.getWeather().id !== 'sandstorm') {
          ...
          for (let sideCondition of ['reflect', 'lightscreen', 'auroraveil']) {
            if (pokemon.side.removeSideCondition(sideCondition)) {
              this.add('-sideend', pokemon.side,
                       ...
                       '[from] ability: Screen Cleaner', '[of] ' + pokemon);
            }
            if (pokemon.side.foe.removeSideCondition(sideCondition)) {
              this.add('-sideend', pokemon.side.foe,
                       this.dex.getEffect(sideCondition).name,
                       '[from] ability: Screen Cleaner', '[of] ' + pokemon);
            }
          }
      ...
      "sheerforce" : {
        ...
        onModifyMove(move, pokemon) {
          if (move.secondaries) {
            ...
            // Actual negation of `AfterMoveSecondary` effects implemented in
            // scripts.js
      ...
      "slowstart" : {
        ...
        onStart(pokemon) { pokemon.addVolatile('slowstart'); },
        onEnd(pokemon) {
        ...
        effect : {
          duration : 5,
          onStart(
              target) { this.add('-start', target, 'ability: Slow Start'); },
          onModifyAtkPriority : 5,
          onModifyAtk(atk, pokemon) { return this.chainModify(0.5); },
          onModifySpe(spe, pokemon) { return this.chainModify(0.5); },
      ...
      "soulheart" : {
        ...
        onAnyFaint() { this.boost({spa : 1}, this.effectData.target); },
        id : "soulheart",
        name : "Soul-Heart",
        rating : 3.5,
        num : 220,
      ...
      "stall" : {
        ...
        onModifyPriority(priority) { return Math.round(priority) - 0.1; },
        id : "stall",
        name : "Stall",
        rating : -1,
        num : 100,
      ...
      "stancechange" : {
        ...
        onBeforeMove(attacker, defender, move) {
          if (attacker.template.baseSpecies !== 'Aegislash' ||
      ...
      "stickyhold" : {
        ...
        onTakeItem(item, pokemon, source) {
          if (this.suppressingAttackEvents() &&
                  pokemon !== this.activePokemon ||
              !pokemon.hp || pokemon.item === 'stickybarb')
            return;
          if (!this.activeMove)
            throw new Error("Battle.activeMove is null");
      ...
      "sweetveil" : {
        ...
        onAllySetStatus(status, target, source, effect) {
          if (status.id === 'slp') {
            ...
            this.add('-activate', this.effectData.target, 'ability: Sweet Veil',
                     '[of] ' + target);
            return null;
          }
        },
      ...
      "symbiosis" : {
        ...
        onAllyAfterUseItem(item, pokemon) {
          ...
          if (!myItem)
            return;
          // @ts-ignore
          if (!this.singleEvent('TakeItem', myItem, source.itemData, pokemon,
                                source, this.effectData, myItem) ||
      ...
      "synchronize" : {
        ...
        onAfterSetStatus(status, target, source, effect) {
          ...
          // Hack to make status-prevention abilities think Synchronize is a
          // status move and show messages when activating against it.
          // @ts-ignore
          source.trySetStatus(status, target,
                              {status : status.id, id : 'synchronize'});
        },
        id : "synchronize",
        name : "Synchronize",
        rating : 2,
      ...
      "teravolt" : {
        shortDesc :
            "This Pokemon's moves and their effects ignore the Abilities of other Pokemon.",
        onStart(pokemon) { this.add('-ability', pokemon, 'Teravolt'); },
        onModifyMove(move) { move.ignoreAbility = true; },
        id : "teravolt",
        name : "Teravolt",
        rating : 3.5,
      ...
      "trace" : {
        ...
        onUpdate(pokemon) {
          ...
          while (possibleTargets.length) {
            ...
            let ability = this.dex.getAbility(target.ability);
            let bannedAbilities = [
              'noability',      'battlebond',     'comatose',
              'disguise',       'flowergift',     'forecast',
              'gulpmissile',    'hungerswitch',   'iceface',
              'illusion',       'imposter',       'multitype',
              'powerconstruct', 'powerofalchemy', 'receiver',
              'rkssystem',      'schooling',      'shieldsdown',
              'stancechange',   'trace',          'zenmode'
            ];
            if (bannedAbilities.includes(target.ability)) {
      ...
      "turboblaze" : {
        shortDesc :
            "This Pokemon's moves and their effects ignore the Abilities of other Pokemon.",
        onStart(pokemon) { this.add('-ability', pokemon, 'Turboblaze'); },
        onModifyMove(move) { move.ignoreAbility = true; },
        id : "turboblaze",
        name : "Turboblaze",
        rating : 3.5,
      ...
      "wanderingspirit" : {
        ...
        onAfterDamage(damage, target, source, move) {
          ...
          let bannedAbilities = [
            'battlebond', 'comatose', 'disguise', 'gulpmissile', 'hungerswitch',
            'iceface', 'illusion', 'multitype', 'powerconstruct', 'rkssystem',
            'schooling', 'shieldsdown', 'stancechange', 'wonderguard', 'zenmode'
          ];
          if (source && source !== target && move && move.flags['contact'] &&
            ...
            if (target.side === source.side) {
              this.add('-activate', source, 'ability: Wandering Spirit', '', '',
                       '[of] ' + target);
            } else {
              this.add('-activate', source, 'ability: Wandering Spirit',
                       targetAbility, sourceAbility, '[of] ' + target);
            }
            ...
            this.singleEvent('Start', targetAbility, source.abilityData,
                             source);
            this.singleEvent('Start', sourceAbility, target.abilityData,
                             target);
          }
        },
        id : "wanderingspirit",
      ...
      "zenmode" : {
        ...
        onResidual(pokemon) {
          ...
          if (pokemon.hp <= pokemon.maxhp / 2 &&
              !['Zen', 'Galar-Zen'].includes(pokemon.template.forme)) {
        ...
        onEnd(pokemon) {
          ...
          pokemon.formeChange(
              /** @type {string} */ (pokemon.template.inheritsFrom),
              this.effect, false, '[silent]');
        },
        effect : {
          onStart(pokemon) {
            if (!pokemon.template.species.includes('Galar')) {
              ...
              if (pokemon.template.speciesid !== 'darmanitangalarzen')
