...
bite: Power is multiplied by 1.5 when used by a Pokemon with the Strong Jaw
Ability. bullet: Has no effect on Pokemon with the Bulletproof Ability. charge:
The user is unable to make a move between turns. contact: Makes contact. dance:
When used by a Pokemon, other Pokemon with the Dancer Ability can attempt to
execute the same move. defrost: Thaws the user if executed successfully while
the user is frozen. distance: Can target a Pokemon positioned anywhere in a
Triple Battle. gravity: Prevented from being executed or selected during
Gravity's effect. heal: Prevented from being executed or selected during Heal
Block's effect. mirror: Can be copied by Mirror Move. mystery: Unknown effect.
nonsky: Prevented from being executed or selected in a Sky Battle.
powder: Has no effect on Grass-type Pokemon, Pokemon with the Overcoat Ability,
and Pokemon holding Safety Goggles. protect: Blocked by Detect, Protect, Spiky
Shield, and if not a Status move, King's Shield. pulse: Power is multiplied
by 1.5 when used by a Pokemon with the Mega Launcher Ability. punch: Power is
multiplied by 1.2 when used by a Pokemon with the Iron Fist Ability. recharge:
If this move is successful, the user must recharge on the following turn and
cannot make a move. reflectable: Bounced back to the original user by Magic Coat
or the Magic Bounce Ability. snatch: Can be stolen from the original user and
instead used by another Pokemon using Snatch. sound: Has no effect on Pokemon
...
let BattleMovedex = {
  ...
  "assist" : {
    ...
    onHit(target) {
      let noAssist = [
        ...
        'destinybond',  'detect',        'dig',         'dive',
        'dragontail',   'endure',        'feint',       'fly',
        'focuspunch',   'followme',      'helpinghand', 'holdhands',
        'kingsshield',  'matblock',      'mefirst',     'metronome',
        'mimic',        'mirrorcoat',    'mirrormove',  'naturepower',
        'phantomforce', 'protect',       'ragepowder',  'roar',
        'shadowforce',  'shelltrap',     'sketch',      'skydrop',
        'sleeptalk',    'snatch',        'spikyshield', 'spotlight',
        'struggle',     'switcheroo',    'thief',       'transform',
        'trick',        'whirlwind',
      ];
  ...
  "attract" : {
    ...
    effect : {
      ...
      onStart(pokemon, source, effect) {
        if (!(pokemon.gender === 'M' && source.gender === 'F') &&
  ...
  "auroraveil" : {
    ...
    effect : {
      ...
      onStart(side) { this.add('-sidestart', side, 'move: Aurora Veil'); },
      onResidualOrder : 21,
      onResidualSubOrder : 1,
      onEnd(side) { this.add('-sideend', side, 'move: Aurora Veil'); },
    },
    secondary :
        null,
    target : "allySide",
  ...
  "autotomize" : {
    ...
    onTryHit(pokemon) {
      ...
      if ((!hasContrary && pokemon.boosts.spe === 6) ||
  ...
  "beakblast" : {
    ...
    shortDesc :
        "Burns on contact with the user before it moves.",
    id : "beakblast",
    isNonstandard :
        'Past',
    name : "Beak Blast",
    ...
    beforeTurnCallback(pokemon) { pokemon.addVolatile('beakblast'); },
    effect : {
      duration : 1,
      onStart(
          pokemon) { this.add('-singleturn', pokemon, 'move: Beak Blast'); },
      onHit(pokemon, source, move) {
        if (move.flags['contact']) {
  ...
  "bellydrum" : {
    ...
    flags : {snatch : 1},
    onHit(target) {
      if (target.hp <= target.maxhp / 2 || target.boosts.atk >= 6 ||
  ...
  "bestow" : {
    ...
    onHit(target, source, move) {
      ...
      if (!myItem)
        return false;
      if (!this.singleEvent('TakeItem', myItem, source.itemData, target, source,
                            move, myItem) ||
  ...
  "bide" : {
    ...
    effect : {
        ...
        if (this.effectData.duration === 1) {
          ...
          if (!target.isActive) {
            const possibleTarget =
                this.resolveTarget(pokemon, this.dex.getMove('pound'));
            if (!possibleTarget) {
      ...
      onMoveAborted(pokemon) { pokemon.removeVolatile('bide'); },
      onEnd(pokemon) { this.add('-end', pokemon, 'move: Bide', '[silent]'); },
    },
    secondary : null,
    target : "self",
    type : "Normal",
  ...
  "bodypress" : {
    ...
    category : "Physical",
    desc :
        "Damage is calculated using the user's Defense stat instead of Attack, including stat stage changes. The user's Ability is used as normal.",
  ...
  "bounce" : {
    ...
    effect : {
      ...
      onInvulnerability(target, source, move) {
        if ([
              'gust', 'twister', 'skyuppercut', 'thunder', 'hurricane',
  ...
  "captivate" : {
    ...
    onTryImmunity(pokemon, source) {
      return (pokemon.gender === 'M' && source.gender === 'F') ||
             (pokemon.gender === 'F' && source.gender === 'M');
    },
    boosts : {
      spa : -2,
  ...
  "celebrate" : {
    ...
    flags : {},
    onTryHit(target,
             source) { this.add('-activate', target, 'move: Celebrate'); },
    secondary : null,
    target : "self",
    type : "Normal",
  ...
  "charge" : {
    ...
    onHit(pokemon) { this.add('-activate', pokemon, 'move: Charge'); },
    effect : {
      duration : 2,
      onRestart(pokemon) { this.effectData.duration = 2; },
  ...
  "clangoroussoul" : {
    ...
    onTryHit(pokemon, target, move) {
      if (pokemon.hp <= pokemon.maxhp / 3 || pokemon.boosts.atk >= 6 ||
          pokemon.boosts.def >= 6 || pokemon.boosts.spa >= 6 ||
          pokemon.boosts.spd >= 6 || pokemon.boosts.spe >= 6 ||
          pokemon.maxhp === 1) {
        delete move.boosts;
        return false;
      }
    },
    onHit(pokemon) { this.directDamage(pokemon.maxhp / 3); },
    boosts : {
      atk : 1,
      def : 1,
      spa : 1,
  ...
  "copycat" : {
    ...
    onHit(pokemon) {
      let noCopycat = [
        'assist',      'banefulbunker', 'beakblast',  'belch',
        'bestow',      'celebrate',     'chatter',    'circlethrow',
        'copycat',     'counter',       'covet',      'craftyshield',
        'destinybond', 'detect',        'dragontail', 'endure',
        ...
        'roar',        'shelltrap',     'sketch',     'sleeptalk',
        'snatch',      'spikyshield',   'spotlight',  'struggle',
  ...
  "coreenforcer" : {
    ...
    onHit(target) {
      if ([
            'battlebond', 'comatose', 'disguise', 'multitype', 'powerconstruct',
            'rkssystem', 'schooling', 'shieldsdown', 'stancechange', 'zenmode'
    ...
    onHitField(target, source) {
      ...
      for (let id of sideConditions) {
        ...
        let targetLayers = side2.sideConditions[id]
                               ...
                               : 0;
        if (sourceLayers === targetLayers)
          continue;
        const effectName = this.dex.getEffect(id).name;
        if (side1.removeSideCondition(id)) {
          this.add('-sideend', side1, effectName, '[from] move: Court Change',
                   '[of] ' + source);
        }
        if (side2.removeSideCondition(id)) {
          this.add('-sideend', side2, effectName, '[from] move: Court Change',
                   '[of] ' + source);
        }
  ...
  "covet" : {
    ...
    onAfterHit(target, source, move) {
      ...
      if (!this.singleEvent('TakeItem', yourItem, target.itemData, source,
                            target, move, yourItem) ||
          !source.setItem(yourItem)) {
        target.item =
            yourItem
                .id; // bypass setItem so we don't break choicelock or anything
        return;
      }
  ...
  "craftyshield" : {
    ...
    onTryHitSide(side, source) { return !!this.willAct(); },
    effect : {
      duration : 1,
      onStart(target,
              source) { this.add('-singleturn', source, 'Crafty Shield'); },
      onTryHitPriority : 3,
      onTryHit(target, source, move) {
  ...
  "crushgrip" : {
    ...
    basePowerCallback(pokemon, target) {
      return Math.floor(Math.floor((120 * (100 * Math.floor(target.hp * 4096 /
                                                            target.maxhp)) +
                                    2048 - 1) /
                                   4096) /
                        100) ||
  ...
  "curse" : {
    ...
    onHit(target,
          source) { this.directDamage(source.maxhp / 2, source, source); },
    effect : {
      onStart(
          pokemon,
          source) { this.add('-start', pokemon, 'Curse', '[of] ' + source); },
      onResidualOrder : 10,
      ...
      for (const sideCondition of removeAll) {
        if (source.side.removeSideCondition(sideCondition)) {
          this.add('-sideend', source.side,
                   this.dex.getEffect(sideCondition).name, '[from] move: Defog',
                   '[of] ' + source);
          success = true;
        }
      }
  ...
  "destinybond" : {
    ...
    effect : {
      ...
      onMoveAborted(pokemon, target,
                    move) { pokemon.removeVolatile('destinybond'); },
      onBeforeSwitchOutPriority : 1,
      onBeforeSwitchOut(pokemon) { pokemon.removeVolatile('destinybond'); },
    },
    secondary : null,
    target : "self",
    type : "Ghost",
  ...
  "disable" : {
    ...
    effect : {
      ...
      noCopy : true, // doesn't get copied by Baton Pass
      onStart(pokemon, source, effect) {
        // The target hasn't taken its turn, or Cursed Body activated and the
        // move was not used through Dancer or Instruct
        if (this.willMove(pokemon) ||
            (pokemon === this.activePokemon && this.activeMove &&
            ...
            if (!moveSlot.pp) {
              ...
              if (effect.id === 'cursedbody') {
                this.add('-start', pokemon, 'Disable', moveSlot.move,
                         '[from] ability: Cursed Body', '[of] ' + source);
              } else {
                this.add('-start', pokemon, 'Disable', moveSlot.move);
              }
    ...
    onModifyMove(move, pokemon, target) {
      if (target.side.active.length === 2) {
        ...
        for (const currentTarget of target.side.active) {
          if (currentTarget.volatiles['protect'] ||
              !currentTarget.runImmunity('Dragon') || currentTarget.fainted) {
            move.multihit = 2;
            break;
          }
        }
  ...
  "echoedvoice" : {
    ...
    priority : 0,
    flags : {protect : 1, mirror : 1, sound : 1, authentic : 1},
    onTry() { this.field.addPseudoWeather('echoedvoice'); },
    effect : {
      duration : 2,
      onStart() { this.effectData.multiplier = 1; },
      onRestart() {
  ...
  "electricterrain" : {
    ...
    effect : {
      ...
      onEnd() { this.add('-fieldend', 'move: Electric Terrain'); },
    },
    secondary : null,
    target : "all",
    type : "Electric",
  ...
  "electrify" : {
    ...
    effect : {
      duration : 1,
      onStart(target) { this.add('-singleturn', target, 'move: Electrify'); },
  ...
  "electroball" : {
    ...
    basePowerCallback(pokemon, target) {
      ...
      this.debug([ 40, 60, 80, 120,
                   150 ][(Math.floor(ratio) > 4 ? 4 : Math.floor(ratio))] +
                 ' bp');
      if (ratio >= 4) {
        return 150;
      }
  ...
  "embargo" : {
    ...
    effect : {
      duration : 5,
      onStart(pokemon) { this.add('-start', pokemon, 'Embargo'); },
      // Item suppression implemented in Pokemon.ignoringItem() within
      // sim/pokemon.js
      onResidualOrder : 18,
  ...
  "encore" : {
    ...
    effect : {
      ...
      onStart(target) {
        ...
        if (!move || move.isZ || move.isMax || noEncore.includes(move.id) ||
      ...
      onResidual(target) {
        if (target.moves.includes(this.effectData.move) &&
            target.moveSlots[target.moves.indexOf(this.effectData.move)].pp <=
                0) {
      ...
      onEnd(target) { this.add('-end', target, 'Encore'); },
      onDisableMove(pokemon) {
  ...
  "endeavor" : {
    ...
    basePower : 0,
    damageCallback(pokemon,
                   target) { return target.getUndynamaxedHP() - pokemon.hp; },
    category : "Physical",
    desc :
  ...
  "entrainment" : {
    ...
    onTryHit(target, source) {
      ...
      let bannedTargetAbilities = [
        'battlebond', 'comatose', 'disguise', 'multitype', 'powerconstruct',
        'rkssystem', 'schooling', 'shieldsdown', 'stancechange', 'truant'
      ];
      let bannedSourceAbilities = [
        'battlebond', 'comatose', 'disguise', 'flowergift', 'forecast',
        'illusion', 'imposter', 'multitype', 'powerconstruct', 'powerofalchemy',
    ...
    onHit(target, source) {
      ...
      if (oldAbility) {
        this.add('-ability', target, this.dex.getAbility(target.ability).name,
  ...
  "eruption" : {
    ...
    basePowerCallback(
        pokemon, target,
        move) { return move.basePower * pokemon.hp / pokemon.maxhp; },
    category : "Special",
    desc :
  ...
  "fairylock" : {
    ...
    effect : {
      duration : 2,
      onStart(target) { this.add('-fieldactivate', 'move: Fairy Lock'); },
  ...
  "firepledge" : {
    ...
    onPrepareHit(target, source, move) {
      for (const action of this.queue) {
        // @ts-ignore
        if (!action.move || !action.pokemon || !action.pokemon.isActive ||
            action.pokemon.fainted)
          continue;
        // @ts-ignore
        if (action.pokemon.side === source.side &&
    ...
    effect : {
      duration : 4,
      onStart(
          targetSide) { this.add('-sidestart', targetSide, 'Fire Pledge'); },
      onEnd(targetSide) { this.add('-sideend', targetSide, 'Fire Pledge'); },
      onResidual(side) {
    ...
    onHit(target, source, move) {
      ...
      for (const ally of target.side.active) {
        if (ally && this.isAdjacent(target, ally)) {
          this.damage(ally.baseMaxhp / 16, ally, source,
  ...
  "fling" : {
    ...
    onPrepareHit(target, source, move) {
      ...
      if (!this.singleEvent('TakeItem', item, source.itemData, source, source,
                            move, item))
        return false;
      if (!item.fling)
        return false;
  ...
  "floralhealing" : {
    ...
    onHit(target, source) {
      ...
      if (this.field.isTerrain('grassyterrain')) {
        success = !!this.heal(
            this.modify(target.maxhp, 0.667)); // TODO: find out the real value
      } else {
  ...
  "flyingpress" : {
    ...
    onEffectiveness(
        typeMod, target, type,
        move) { return typeMod + this.dex.getEffectiveness('Flying', type); },
    priority : 0,
    secondary : null,
    target : "any",
    type : "Fighting",
  ...
  "focusenergy" : {
    ...
    effect : {
      onStart(target, source, effect) {
        ...
        } else if (effect && ([ 'imposter', 'psychup', 'transform' ].includes(
                                 effect.id))) {
  ...
  "focuspunch" : {
    ...
    beforeTurnCallback(pokemon) { pokemon.addVolatile('focuspunch'); },
    beforeMoveCallback(pokemon) {
    ...
    effect : {
      duration : 1,
      onStart(
          pokemon) { this.add('-singleturn', pokemon, 'move: Focus Punch'); },
      onHit(pokemon, source, move) {
  ...
  "followme" : {
    ...
    effect : {
      ...
      onFoeRedirectTarget(target, source, source2, move) {
        if (!this.effectData.target.isSkyDropped() &&
  ...
  "forestscurse" : {
    ...
    onHit(target) {
      ...
      if (!target.addType('Grass'))
        return false;
      this.add('-start', target, 'typeadd', 'Grass',
  ...
  "furycutter" : {
    ...
    basePowerCallback(pokemon, target, move) {
      ...
      return this.dex.clampIntRange(
          move.basePower * pokemon.volatiles.furycutter.multiplier, 1, 160);
    },
    category : "Physical",
    desc :
  ...
  "gastroacid" : {
    ...
    effect : {
      // Ability suppression implemented in Pokemon.ignoringAbility() within
      // sim/pokemon.js
      onStart(pokemon) {
        this.add('-endability', pokemon);
        this.singleEvent('End', this.dex.getAbility(pokemon.ability),
                         pokemon.abilityData, pokemon, pokemon, 'gastroacid');
      },
    },
    secondary : null,
    target : "normal",
  ...
  "gmaxcentiferno" : {
    ...
    self : {
      onHit(source) {
        for (let pokemon of source.side.foe.active) {
          pokemon.addVolatile('partiallytrapped', source,
  ...
  "gmaxchistrike" : {
    ...
    category : "Physical",
    desc :
        "Applies Focus Energy to the user and its allies. Base Power scales with the base move's Base Power.",
  ...
  "gmaxdepletion" : {
    ...
    category : "Physical",
    desc :
        "Lowers the PP of the opponent(s) last used move. Base Power scales with the base move's Base Power.",
      ...
      onAfterHit(source) {
        for (let pokemon of source.side.foe.active) {
          ...
          if (move && !move.isZ && !move.isMax) {
            ...
            if (ppDeducted) {
              this.add("-activate", pokemon, 'move: Max Depletion', move.name,
                       ppDeducted);
              return;
            }
          }
      ...
      onHit(source) {
        ...
        for (let pokemon of source.side.active) {
          if (!pokemon.item && pokemon.lastItem &&
            ...
            pokemon.lastItem = '';
            this.add('-item', pokemon, this.dex.getItem(item),
  ...
  "gmaxsteelsurge" : {
    ...
    isMax : "Copperajah",
    self : {
      onHit(source) { source.side.foe.addSideCondition('gmaxsteelsurge'); },
    },
    effect : {
      onStart(side) { this.add('-sidestart', side, 'move: Steelsurge'); },
      onSwitchIn(pokemon) {
        if (pokemon.hasItem('heavydutyboots'))
          return;
        let typeMod = this.dex.clampIntRange(
            pokemon.runEffectiveness(
                this.dex.getActiveMove('G-Max Steelsurge')),
            -6, 6);
  ...
  "gmaxstonesurge" : {
    ...
    isMax : "Drednaw",
    self : {
      onHit(source) { source.side.foe.addSideCondition('stealthrock'); },
    },
    secondary : null,
    target : "adjacentFoe",
    type : "Water",
  ...
  "gmaxvolcalith" : {
    ...
    category : "Physical",
    desc :
        "Damages opponent(s) by 1/8 of their maximum HP for four turns. Base Power scales with the base move's Base Power.",
    ...
    isMax : "Coalossal",
    self : {
      onHit(source) { source.side.foe.addSideCondition('gmaxvolcalith'); },
    },
    effect : {
      duration : 4,
      onStart(targetSide) {
      ...
      onEnd(
          targetSide) { this.add('-sideend', targetSide, 'G-Max Volcalith'); },
    },
    secondary : null,
    target : "adjacentFoe",
    type : "Rock",
  ...
  "gmaxwildfire" : {
    ...
    category : "Physical",
    desc :
        "Damages non-Fire-type opponent(s) by 1/6 of their maximum HP for four turns. Base Power scales with the base move's Base Power.",
    shortDesc : "Damages foes for 4 turns. BP scales w/ base move.",
    id : "gmaxwildfire",
    isNonstandard : "Custom",
    name : "G-Max Wildfire",
    ...
    isMax : "Charizard",
    self : {
      onHit(source) { source.side.foe.addSideCondition('gmaxwildfire'); },
    },
    effect : {
      duration : 4,
      onStart(
          targetSide) { this.add('-sidestart', targetSide, 'G-Max Wildfire'); },
      onResidual(targetSide) {
        for (const pokemon of targetSide.active) {
          if (!pokemon.hasType('Fire'))
      ...
      onEnd(targetSide) { this.add('-sideend', targetSide, 'G-Max Wildfire'); },
    },
    secondary : null,
    target : "adjacentFoe",
    type : "Fire",
  ...
  "gmaxwindrage" : {
    ...
    basePower : 10,
    category : "Physical",
    // TODO: Test what G-Max Wind Rage actually removes from the field and from
    // which sides
    desc :
        "Removes Reflect, Light Screen, Aurora Veil, Spikes, Toxic Spikes, Stealth Rock, Sticky Web, Mist, Safeguard, G-Max Steelsurge, and Terrains from the field. This move's Base Power scales with the base move's Base Power.",
      ...
      onHit(source) {
        ...
        for (const targetCondition of removeTarget) {
          if (source.side.foe.removeSideCondition(targetCondition)) {
            if (!removeAll.includes(targetCondition))
              continue;
            this.add('-sideend', source.side.foe,
                     this.dex.getEffect(targetCondition).name,
                     '[from] move: G-Max Wind Rage', '[of] ' + source);
            success = true;
          }
        }
        for (const sideCondition of removeAll) {
  ...
  "grasspledge" : {
    ...
    effect : {
      duration : 4,
      onStart(
          targetSide) { this.add('-sidestart', targetSide, 'Grass Pledge'); },
      onEnd(targetSide) { this.add('-sideend', targetSide, 'Grass Pledge'); },
  ...
  "grasswhistle" : {
    ...
    priority : 0,
    flags :
        {protect : 1, reflectable : 1, mirror : 1, sound : 1, authentic : 1},
    status : 'slp',
    secondary : null,
    target : "normal",
    type : "Grass",
  ...
  "grassyterrain" : {
    ...
    effect : {
      ...
      onResidual() { this.eachEvent('Terrain'); },
      onTerrain(pokemon) {
  ...
  "gravity" : {
    ...
    effect : {
      ...
      onStart() {
        ...
        for (const pokemon of this.getAllActive()) {
          let applies = false;
          if (pokemon.removeVolatile('bounce') ||
          ...
          if (pokemon.volatiles['skydrop']) {
            ...
            if (pokemon.volatiles['skydrop'].source) {
              this.add('-end', pokemon.volatiles['twoturnmove'].source,
                       'Sky Drop', '[interrupt]');
            }
      ...
      onResidualOrder : 22,
      onEnd() { this.add('-fieldend', 'move: Gravity'); },
    },
    secondary : null,
    target : "all",
    type : "Psychic",
  ...
  "guardianofalola" : {
    ...
    damageCallback(pokemon, target) {
      ...
      if (target.volatiles['banefulbunker'] ||
          target.volatiles['kingsshield'] ||
          target.side.getSideCondition('matblock') ||
          target.volatiles['protect'] || target.volatiles['spikyshield']) {
  ...
  "guardsplit" : {
    ...
    onHit(target, source) {
      let newdef =
          Math.floor((target.storedStats.def + source.storedStats.def) / 2);
      target.storedStats.def = newdef;
      source.storedStats.def = newdef;
      let newspd =
          Math.floor((target.storedStats.spd + source.storedStats.spd) / 2);
  ...
  "guardswap" : {
    ...
    onHit(target, source) {
      ...
      this.add('-swapboost', source, target, 'def, spd',
  ...
  "happyhour" : {
    ...
    flags : {},
    onTryHit(target,
             source) { this.add('-activate', target, 'move: Happy Hour'); },
    secondary : null,
    target : "allySide",
    type : "Normal",
    zMoveBoost : {atk : 1, def : 1, spa : 1, spd : 1, spe : 1},
  ...
  "healblock" : {
    ...
    effect : {
      ...
      onStart(pokemon) { this.add('-start', pokemon, 'move: Heal Block'); },
      onDisableMove(pokemon) {
        for (const moveSlot of pokemon.moveSlots) {
  ...
  "hiddenpower" : {
    ...
    priority : 0,
    flags : {protect : 1, mirror : 1},
    onModifyType(move, pokemon) { move.type = pokemon.hpType || 'Dark'; },
    secondary : null,
    target : "normal",
    type : "Normal",
    contestType : "Clever",
  ...
  "iceball" : {
    ...
    effect : {
      duration : 2,
      onLockMove : 'iceball',
      onStart() { this.effectData.hitCount = 1; },
      onRestart() {
  ...
  "instruct" : {
    ...
    onHit(target, source) {
      ...
      const noInstruct = [
        'assist',     'beakblast', 'bide',       'celebrate',   'copycat',
        'focuspunch', 'iceball',   'instruct',   'kingsshield', 'mefirst',
        'metronome',  'mimic',     'mirrormove', 'naturepower', 'outrage',
        'petaldance', 'rollout',   'shelltrap',  'sketch',      'sleeptalk',
        'thrash',     'transform',
      ];
      ...
      this.runMove(target.lastMove.id, target,
  ...
  "iondeluge" : {
    ...
    effect : {
      duration : 1,
      onStart(target) { this.add('-fieldactivate', 'move: Ion Deluge'); },
      onModifyTypePriority : -2,
      onModifyType(move) {
        if (move.type === 'Normal') {
          move.type = 'Electric';
  ...
  "knockoff" : {
    ...
    onBasePower(basePower, source, target, move) {
      let item = target.getItem();
      if (!this.singleEvent('TakeItem', item, target.itemData, target, target,
                            move, item))
        return;
      if (item.id) {
  ...
  "lastresort" : {
    ...
    onTryHit(target, source) {
      if (source.moveSlots.length < 2)
        return false; // Last Resort fails unless the user knows at least 2
                      // moves
      let hasLastResort =
          false; // User must actually have Last Resort for it to succeed
      for (const moveSlot of source.moveSlots) {
  ...
  "leechseed" : {
    ...
    effect : {
      ...
      onResidualOrder : 8,
      onResidual(pokemon) {
        let target = this.effectData.source.side
                         .active[pokemon.volatiles['leechseed'].sourcePosition];
        if (!target || target.fainted || target.hp <= 0) {
  ...
  "lightscreen" : {
    ...
    effect : {
      ...
      onStart(side) { this.add('-sidestart', side, 'move: Light Screen'); },
      onResidualOrder : 21,
      onResidualSubOrder : 1,
      onEnd(side) { this.add('-sideend', side, 'move: Light Screen'); },
    },
    secondary : null,
    target : "allySide",
    type : "Psychic",
  ...
  "luckychant" : {
    ...
    effect : {
      ...
      onStart(side) {
        this.add(
            '-sidestart', side,
            'move: Lucky Chant'); // "The Lucky Chant shielded [side.name]'s
      ...
      onEnd(side) {
        this.add('-sideend', side,
                 'move: Lucky Chant'); // "[side.name]'s team's Lucky Chant wore
                                       // off!"
      },
    },
    secondary : null,
  ...
  "lunardance" : {
    ...
    effect : {
      ...
      onRestart(side,
                source) { this.effectData.positions[source.position] = true; },
      onSwitchInPriority : 1,
      onSwitchIn(target) {
  ...
  "magicroom" : {
    ...
    effect : {
      ...
      onRestart(target,
                source) { this.field.removePseudoWeather('magicroom'); },
      // Item suppression implemented in Pokemon.ignoringItem() within
      // sim/pokemon.js
      onResidualOrder : 25,
      onEnd() {
  ...
  "matblock" : {
    ...
    effect : {
      duration : 1,
      onStart(target, source) { this.add('-singleturn', source, 'Mat Block'); },
      onTryHitPriority : 3,
      onTryHit(target, source, move) {
        if (!move.flags['protect']) {
          if (move.isZ || move.isMax)
  ...
  "maxairstream" : {
    ...
    category : "Physical",
    desc :
        "Boosts the user and its allies' Speed by 1 stage. BP scales with the base move's BP.",
  ...
  "maxdarkness" : {
    ...
    category : "Physical",
    desc :
        "Lowers all the opposing Pokemon's Special Defense by 1 stage. BP scales with the base move's BP.",
  ...
  "maxflutterby" : {
    ...
    category : "Physical",
    desc :
        "Lowers all the opposing Pokemon's Special Attack by 1 stage. BP scales with the base move's BP.",
  ...
  "maxknuckle" : {
    ...
    category : "Physical",
    desc :
        "Boosts the user and its allies' Attack by 1 stage. BP scales with the base move's BP.",
  ...
  "maxooze" : {
    ...
    category : "Physical",
    desc :
        "Boosts the user and its allies' Special Attack by 1 stage. BP scales with the base move's BP.",
  ...
  "maxphantasm" : {
    ...
    category : "Physical",
    desc :
        "Lowers all the opposing Pokemon's Defense by 1 stage. BP scales with the base move's BP.",
  ...
  "maxquake" : {
    ...
    category : "Physical",
    desc :
        "Boosts the user and its allies' Special Defense by 1 stage. BP scales with the base move's BP.",
  ...
  "maxsteelspike" : {
    ...
    category : "Physical",
    desc :
        "Boosts the user and its allies' Defense by 1 stage. BP scales with the base move's BP.",
  ...
  "maxstrike" : {
    ...
    category : "Physical",
    desc :
        "Lowers all the opposing Pokemon's Speed by 1 stage. BP scales with the base move's BP.",
  ...
  "maxwyrmwind" : {
    ...
    category : "Physical",
    desc :
        "Lowers all the opposing Pokemon's Attack by 1 stage. BP scales with the base move's BP.",
  ...
  "metalburst" : {
    ...
    priority : 0,
    flags : {protect : 1, mirror : 1},
    beforeTurnCallback(pokemon) { pokemon.addVolatile('metalburst'); },
    onTryHit(target, source, move) {
  ...
  "metronome" : {
    ...
    category : "Status",
    desc :
        "A random move is selected for use, other than After You, Assist, Baneful Bunker, Beak Blast, Behemoth Bash, Behemoth Blade, Belch, Bestow, Celebrate, Chatter, Copycat, Counter, Covet, Crafty Shield, Destiny Bond, Detect, Diamond Storm, Dynamax Cannon, Endure, Eternabeam, Feint, Fleur Cannon, Focus Punch, Follow Me, Freeze Shock, Helping Hand, Hold Hands, Hyperspace Fury, Hyperspace Hole, Ice Burn, Instruct, King's Shield, Light of Ruin, Mat Block, Me First, Metronome, Mimic, Mind Blown, Mirror Coat, Mirror Move, Nature Power, Origin Pulse, Photon Geyser, Plasma Fists, Precipice Blades, Protect, Quash, Quick Guard, Rage Powder, Relic Song, Secret Sword, Shell Trap, Sketch, Sleep Talk, Snarl, Snatch, Snore, Spectral Thief, Spiky Shield, Spotlight, Steam Eruption, Struggle, Switcheroo, Techno Blast, Thief, Thousand Arrows, Thousand Waves, Transform, Trick, V-create, or Wide Guard.",
    ...
    flags : {},
    noMetronome : [
      'afteryou',       'assist',          'banefulbunker',  'beakblast',
      'behemothbash',   'behemothblade',   'belch',          'bestow',
      'celebrate',      'chatter',         'copycat',        'counter',
      'covet',          'craftyshield',    'destinybond',    'detect',
      'diamondstorm',   'dragonascent',    'dynamaxcannon',  'endure',
      'eternabeam',     'feint',           'fleurcannon',    'focuspunch',
      'followme',       'freezeshock',     'helpinghand',    'holdhands',
      'hyperspacefury', 'hyperspacehole',  'iceburn',        'instruct',
      'kingsshield',    'lightofruin',     'matblock',       'mefirst',
      'metronome',      'mimic',           'mindblown',      'mirrorcoat',
      'mirrormove',     'naturepower',     'originpulse',    'photongeyser',
      'plasmafists',    'precipiceblades', 'protect',        'quash',
      'quickguard',     'ragepowder',      'relicsong',      'secretsword',
      'shelltrap',      'sketch',          'sleeptalk',      'snarl',
      'snatch',         'snore',           'spectralthief',  'spikyshield',
  ...
  "mimic" : {
    ...
    onHit(target, source) {
      const disallowedMoves =
          [ 'chatter', 'mimic', 'sketch', 'struggle', 'transform' ];
      const move = target.lastMove;
      if (source.transformed || !move || disallowedMoves.includes(move.id) ||
  ...
  "mindblown" : {
    ...
    onAfterMove(pokemon, target, move) {
      if (move.mindBlownRecoil && !move.multihit) {
        this.damage(Math.round(pokemon.maxhp / 2), pokemon, pokemon,
                    this.dex.getEffect('Mind Blown'), true);
      }
    },
    secondary : null,
    target : "allAdjacent",
    ...
    effect : {
      ...
      onSourceModifyDamage(damage, source, target, move) {
        if ([
              ...
              'heatcrash', 'heavyslam', 'maliciousmoonsault'
            ].includes(move.id)) {
          return this.chainModify(2);
        }
      },
  ...
  "mist" : {
    ...
    effect : {
      ...
      onStart(side) { this.add('-sidestart', side, 'Mist'); },
      onResidualOrder : 21,
      onResidualSubOrder : 3,
      onEnd(side) { this.add('-sideend', side, 'Mist'); },
    },
    secondary : null,
    target : "allySide",
    type : "Ice",
  ...
  "mistyterrain" : {
    ...
    effect : {
      ...
      onResidualOrder : 21,
      onResidualSubOrder : 2,
      onEnd(side) { this.add('-fieldend', 'Misty Terrain'); },
    },
    secondary : null,
    target : "all",
    type : "Fairy",
  ...
  "mudsport" : {
    ...
    effect : {
      ...
      onResidualOrder : 21,
      onEnd() { this.add('-fieldend', 'move: Mud Sport'); },
    },
    secondary : null,
    target : "all",
    type : "Ground",
  ...
  "naturesmadness" : {
    ...
    basePower : 0,
    damageCallback(pokemon, target) {
      return this.dex.clampIntRange(Math.floor(target.getUndynamaxedHP() / 2),
                                    1);
    },
    category : "Special",
    desc :
  ...
  "noretreat" : {
    ...
    onTryMove(pokemon, target, move) {
      if (!pokemon.volatiles['noretreat'])
        return;
  ...
  "obstruct" : {
    ...
    category : "Status",
    desc :
        "The user is protected from attacks made by other Pokemon during this turn, and Pokemon trying to make contact with the user have their Defense lowered by 2 stages. This move has a 1/X chance of being successful, where X starts at 1 and triples each time this move is successfully used. X resets to 1 if this move fails, if the user's last move used is not Baneful Bunker, Detect, Endure, King's Shield, Obstruct, Protect, Quick Guard, Spiky Shield, or Wide Guard, or if it was one of those moves and the user's protection was broken. Fails if the user moves last this turn.",
    ...
    effect : {
      ...
      onResidual(pokemon) {
        ...
        if (source &&
          ...
          this.add('-end', pokemon, 'Octolock', '[partiallytrapped]',
                   '[silent]');
          return;
        }
  ...
  "odorsleuth" : {
    ...
    priority : 0,
    flags :
        {protect : 1, reflectable : 1, mirror : 1, authentic : 1, mystery : 1},
    volatileStatus : 'foresight',
    onTryHit(target) {
      if (target.volatiles['miracleeye'])
        return false;
  ...
  "payday" : {
    ...
    priority : 0,
    flags : {protect : 1, mirror : 1},
    onHit() { this.add('-fieldactivate', 'move: Pay Day'); },
    secondary : null,
    target : "normal",
    type : "Normal",
    contestType : "Clever",
  ...
  "pikapapow" : {
    ...
    basePower : 0,
    basePowerCallback(
        pokemon) { return Math.floor((pokemon.happiness * 10) / 25) || 1; },
    category : "Special",
    desc :
  ...
  "powder" : {
    ...
    priority : 1,
    flags :
        {powder : 1, protect : 1, reflectable : 1, mirror : 1, authentic : 1},
  ...
  "powersplit" : {
    ...
    onHit(target, source) {
      let newatk =
          Math.floor((target.storedStats.atk + source.storedStats.atk) / 2);
      target.storedStats.atk = newatk;
      source.storedStats.atk = newatk;
      let newspa =
          Math.floor((target.storedStats.spa + source.storedStats.spa) / 2);
  ...
  "powerswap" : {
    ...
    onHit(target, source) {
      ...
      this.add('-swapboost', source, target, 'atk, spa',
  ...
  "powertrip" : {
    ...
    basePowerCallback(
        pokemon, target,
        move) { return move.basePower + 20 * pokemon.positiveBoosts(); },
    category : "Physical",
    desc :
  ...
  "psychicterrain" : {
    ...
    effect : {
      ...
      onResidualOrder : 21,
      onResidualSubOrder : 2,
      onEnd() { this.add('-fieldend', 'move: Psychic Terrain'); },
    },
    secondary : null,
    target : "all",
    type : "Psychic",
  ...
  "psychoshift" : {
    ...
    self : {
      onHit(pokemon) { pokemon.cureStatus(); },
    },
    secondary : null,
    target : "normal",
    type : "Psychic",
  ...
  "psywave" : {
    ...
    basePower : 0,
    damageCallback(
        pokemon) { return (this.random(50, 151) * pokemon.level) / 100; },
    category : "Special",
    desc :
    ...
    effect : {
      ...
      onBeforeSwitchOut(pokemon) {
        ...
        for (const source of this.effectData.sources) {
          ...
          // Run through each action in queue to check if the Pursuit user is
          // supposed to Mega Evolve this turn. If it is, then Mega Evolve
          // before moving.
  ...
  "quickguard" : {
    ...
    effect : {
      duration : 1,
      onStart(target,
              source) { this.add('-singleturn', source, 'Quick Guard'); },
      onTryHitPriority : 4,
      onTryHit(target, source, move) {
        // Quick Guard blocks moves with positive priority, even those given
        // increased priority by Prankster or Gale Wings. (e.g. it blocks 0
        // priority moves boosted by Prankster or Gale Wings; Quick Claw/Custap
        // Berry do not count)
  ...
  "ragepowder" : {
    ...
    effect : {
      duration : 1,
      onStart(
          pokemon) { this.add('-singleturn', pokemon, 'move: Rage Powder'); },
      onFoeRedirectTargetPriority : 1,
      onFoeRedirectTarget(target, source, source2, move) {
        if (!this.effectData.target.isSkyDropped() &&
  ...
  "reflect" : {
    ...
    effect : {
      ...
      onStart(side) { this.add('-sidestart', side, 'Reflect'); },
      onResidualOrder : 21,
      onEnd(side) { this.add('-sideend', side, 'Reflect'); },
    },
    secondary : null,
    target : "allySide",
    type : "Psychic",
  ...
  "relicsong" : {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (move.willChangeForme) {
        pokemon.formeChange(pokemon.template.speciesid === 'meloettapirouette'
                                ? 'Meloetta'
                                : 'Meloetta-Pirouette',
                            this.effect, false, '[msg]');
      }
    },
    target : "allAdjacentFoes",
    type : "Normal",
  ...
  "rest" : {
    ...
    onHit(target, source, move) {
      ...
      this.heal(target.maxhp); // Aesthetic only as the healing happens after
                               // you fall asleep in-game
    },
    secondary : null,
    target : "self",
    type : "Psychic",
  ...
  "roar" : {
    ...
    priority : -6,
    flags :
        {reflectable : 1, mirror : 1, sound : 1, authentic : 1, mystery : 1},
    forceSwitch : true,
    secondary : null,
    target : "normal",
    type : "Normal",
  ...
  "roleplay" : {
    ...
    onHit(target, source) {
      ...
      if (oldAbility) {
        this.add('-ability', source, this.dex.getAbility(source.ability).name,
                 '[from] move: Role Play', '[of] ' + target);
        return;
      }
      return false;
    },
  ...
  "roost" : {
    ...
    effect : {
      duration : 1,
      onResidualOrder : 20,
      onStart(target) { this.add('-singleturn', target, 'move: Roost'); },
      onTypePriority : -1,
  ...
  "rototiller" : {
    ...
    onHitField(target, source) {
      ...
      if (!targets.length && !anyAirborne)
        return false; // Fails when there are no grounded Grass types or
                      // airborne Pokemon
  ...
  "safeguard" : {
    ...
    effect : {
      ...
      onStart(side) { this.add('-sidestart', side, 'Safeguard'); },
      onResidualOrder : 21,
      onResidualSubOrder : 2,
      onEnd(side) { this.add('-sideend', side, 'Safeguard'); },
    },
    secondary : null,
    target : "allySide",
    type : "Normal",
  ...
  "shelltrap" : {
    ...
    priority : -3,
    flags : {protect : 1},
    beforeTurnCallback(pokemon) { pokemon.addVolatile('shelltrap'); },
    // TODO: In order to correct PP usage, after spread move order has been
    // reworked,
    // switch this to `onTry` + add `this.attrLastMove('[still]');`.
    beforeMoveCallback(pokemon) {
      if (!pokemon.volatiles['shelltrap'] ||
    ...
    effect : {
      duration : 1,
      onStart(
          pokemon) { this.add('-singleturn', pokemon, 'move: Shell Trap'); },
      onHit(pokemon, source, move) {
  ...
  "simplebeam" : {
    ...
    onTryHit(pokemon) {
      let bannedAbilities = [
        'battlebond', 'comatose', 'disguise', 'multitype', 'powerconstruct',
        'rkssystem', 'schooling', 'shieldsdown', 'simple', 'stancechange',
        'truant', 'zenmode'
      ];
      if (bannedAbilities.includes(pokemon.ability)) {
        return false;
  ...
  "skillswap" : {
    ...
    onTryHit(target, source) {
      let bannedAbilities = [
        'battlebond', 'comatose', 'disguise', 'illusion', 'multitype',
        'powerconstruct', 'rkssystem', 'schooling', 'shieldsdown',
        'stancechange', 'wonderguard', 'zenmode'
      ];
      if (target.volatiles['dynamax'] ||
    ...
    onHit(target, source, move) {
      ...
      if (target.side === source.side) {
        ...
        this.add('-activate', source, 'move: Skill Swap', targetAbility,
                 sourceAbility, '[of] ' + target);
      }
  ...
  "skydrop" : {
    ...
    onHit(target, source) { this.add('-end', target, 'Sky Drop'); },
    effect : {
      duration : 2,
      ...
      onFaint(target) {
        if (target.volatiles['skydrop'] &&
          ...
          this.add('-end', target.volatiles['twoturnmove'].source, 'Sky Drop',
                   '[interrupt]');
        }
      },
    },
  ...
  "spitup" : {
    ...
    onAfterMove(pokemon) { pokemon.removeVolatile('stockpile'); },
    secondary : null,
    target : "normal",
    type : "Normal",
    contestType : "Tough",
  ...
  "stealthrock" : {
    ...
    effect : {
      // this is a side condition
      onStart(side) { this.add('-sidestart', side, 'move: Stealth Rock'); },
      onSwitchIn(pokemon) {
        if (pokemon.hasItem('heavydutyboots'))
          return;
        let typeMod = this.dex.clampIntRange(
            pokemon.runEffectiveness(this.dex.getActiveMove('stealthrock')), -6,
            6);
        this.damage(pokemon.maxhp * Math.pow(2, typeMod) / 8);
      },
    },
  ...
  "steelbeam" : {
    ...
    onAfterMove(pokemon, target, move) {
      if (move.mindBlownRecoil && !move.multihit) {
        this.damage(Math.round(pokemon.maxhp / 2), pokemon, pokemon,
                    this.dex.getEffect('Steel Beam'), true);
      }
    },
    secondary : null,
    target : "normal",
  ...
  "stickyweb" : {
    ...
    effect : {
      onStart(side) { this.add('-sidestart', side, 'move: Sticky Web'); },
      onSwitchIn(pokemon) {
        if (!pokemon.isGrounded())
          return;
        if (pokemon.hasItem('heavydutyboots'))
  ...
  "stockpile" : {
    ...
    effect : {
      ...
      onEnd(target) {
        ...
        if (this.effectData.def !== this.effectData.layers * -1 ||
  ...
  "substitute" : {
    ...
    effect : {
      ...
      onTryPrimaryHit(target, source, move) {
        ...
        if (move.recoil) {
          this.damage(this.calcRecoilDamage(damage, move), source, target,
                      'recoil');
        }
        if (move.drain) {
          this.heal(Math.ceil(damage * move.drain[0] / move.drain[1]), source,
                    target, 'drain');
        }
  ...
  "switcheroo" : {
    ...
    onHit(target, source, move) {
      ...
      if ((myItem && !this.singleEvent('TakeItem', myItem, source.itemData,
                                       target, source, move, myItem)) ||
          (yourItem && !this.singleEvent('TakeItem', yourItem, target.itemData,
                                         source, target, move, yourItem))) {
        if (yourItem)
          target.item = yourItem.id;
        if (myItem)
          source.item = myItem.id;
  ...
  "tailwind" : {
    ...
    effect : {
      ...
      onStart(side) { this.add('-sidestart', side, 'move: Tailwind'); },
      onModifySpe(spe, pokemon) { return this.chainModify(2); },
      onResidualOrder : 21,
      onResidualSubOrder : 4,
      onEnd(side) { this.add('-sideend', side, 'move: Tailwind'); },
    },
    secondary : null,
    target : "allySide",
    type : "Flying",
  ...
  "tarshot" : {
    ...
    effect : {
      ...
      onEffectiveness(typeMod, target, type, move) {
        if (move.type !== 'Fire')
          return;
        if (!target || !target.volatiles['tarshot'])
          return;
        return typeMod + 1;
      },
      onResidualOrder : 21,
      onEnd(pokemon) { this.add('-end', pokemon, 'Tar Shot'); },
    },
    secondary : null,
    target : "normal",
    type : "Rock",
      ...
      for (const active of this.getAllActive()) {
        if (!this.runEvent('Invulnerability', active, source, move)) {
          ...
          if (active.hp && item.isBerry) {
            ...
            this.add('-enditem', target, item.name, '[from] eat',
                     '[move] Teatime', '[of] ' + active);
            if (this.singleEvent('Eat', item, null, active, null, null)) {
              this.runEvent('EatItem', active, null, null, item);
              // TODO: Test
              if (item.id === 'leppaberry')
                active.staleness = 'external';
            }
            if (item.onEat)
              active.ateBerry = true;
            result = true;
          }
        }
      }
  ...
  "telekinesis" : {
    ...
    priority : 0,
    flags :
        {protect : 1, reflectable : 1, mirror : 1, gravity : 1, mystery : 1},
    volatileStatus : 'telekinesis',
    effect : {
      duration : 3,
      onStart(target) {
        if ([ 'Diglett', 'Dugtrio', 'Palossand', 'Sandygast' ].includes(
  ...
  "throatchop" : {
    ...
    effect : {
      duration : 2,
      onStart(
          target) { this.add('-start', target, 'Throat Chop', '[silent]'); },
      onDisableMove(pokemon) {
        for (const moveSlot of pokemon.moveSlots) {
      ...
      onResidualOrder : 22,
      onEnd(target) { this.add('-end', target, 'Throat Chop', '[silent]'); },
    },
    secondary : {
      chance : 100,
      onHit(target) { target.addVolatile('throatchop'); },
    },
    target : "normal",
    type : "Dark",
    contestType : "Clever",
  ...
  "torment" : {
    ...
    effect : {
      noCopy : true,
      onStart(pokemon) { this.add('-start', pokemon, 'Torment'); },
  ...
  "toxic" : {
    ...
    priority : 0,
    flags : {protect : 1, reflectable : 1, mirror : 1},
    // No Guard-like effect for Poison-type users implemented in
    // BattleScripts#tryMoveHit
    status : 'tox',
    secondary : null,
    target : "normal",
    type : "Poison",
  ...
  "trickroom" : {
    ...
    effect : {
      ...
      onRestart(target,
                source) { this.field.removePseudoWeather('trickroom'); },
      // Speed modification is changed in Pokemon.getActionSpeed() in
      // sim/pokemon.js
      onResidualOrder : 23,
      onEnd() { this.add('-fieldend', 'move: Trick Room'); },
    },
    secondary : null,
    target : "all",
    type : "Psychic",
  ...
  "triplekick" : {
    ...
    accuracy : 90,
    basePower : 10,
    basePowerCallback(pokemon, target, move) { return 10 * move.hit; },
    category : "Physical",
    desc :
  ...
  "trumpcard" : {
    ...
    basePowerCallback(source, target, move) {
      ...
      const moveSlot = callerMoveId === 'instruct'
  ...
  "venomdrench" : {
    ...
    onHit(target, source, move) {
      if (target.status === 'psn' || target.status === 'tox') {
        return !!this.boost({atk : -1, spa : -1, spe : -1}, target, source,
                            move);
      }
      return false;
    },
  ...
  "waterpledge" : {
    ...
    effect : {
      duration : 4,
      onStart(
          targetSide) { this.add('-sidestart', targetSide, 'Water Pledge'); },
      onEnd(targetSide) { this.add('-sideend', targetSide, 'Water Pledge'); },
      onModifyMove(move) {
  ...
  "watershuriken" : {
    ...
    basePower : 15,
    basePowerCallback(pokemon, target, move) {
      if (pokemon.template.species === 'Greninja-Ash' &&
  ...
  "watersport" : {
    ...
    effect : {
      ...
      onResidualOrder : 21,
      onEnd() { this.add('-fieldend', 'move: Water Sport'); },
    },
    secondary : null,
    target : "all",
    type : "Water",
  ...
  "wideguard" : {
    ...
    effect : {
      duration : 1,
      onStart(target,
              source) { this.add('-singleturn', source, 'Wide Guard'); },
      onTryHitPriority : 4,
      onTryHit(target, source, move) {
        // Wide Guard blocks all spread moves
        if (move && move.target !== 'allAdjacent' &&
            move.target !== 'allAdjacentFoes') {
          return;
        }
        if (move.isZ || move.isMax) {
  ...
  "wish" : {
    ...
    effect : {
      duration : 2,
      onStart(pokemon, source) { this.effectData.hp = source.maxhp / 2; },
      onResidualOrder : 4,
      onEnd(target) {
  ...
  "wonderroom" : {
    ...
    effect : {
      ...
      onRestart(target,
                source) { this.field.removePseudoWeather('wonderroom'); },
      // Swapping defenses implemented in sim/pokemon.js:Pokemon#calculateStat
      // and Pokemon#getStat
      onResidualOrder : 24,
      onEnd() { this.add('-fieldend', 'move: Wonder Room'); },
    },
    secondary : null,
    target : "all",
    type : "Psychic",
  ...
  "worryseed" : {
    ...
    onTryHit(pokemon) {
      let bannedAbilities = [
        'battlebond', 'comatose', 'disguise', 'insomnia', 'multitype',
        'powerconstruct', 'rkssystem', 'schooling', 'shieldsdown',
        'stancechange', 'truant', 'zenmode'
      ];
      if (bannedAbilities.includes(pokemon.ability)) {
        return false;
      }
