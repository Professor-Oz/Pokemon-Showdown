...
let BattleMovedex = {
	...
	boltbeak: {
		...
		category: "Physical",
		desc:
			"If the user moves before the target, this move's power is doubled.",
	...
	clangoroussoul: {
		...
		onTryHit(pokemon, target, move) {
			if (
				...
				pokemon.boosts.def >= 6 ||
				pokemon.boosts.spa >= 6 ||
				pokemon.boosts.spd >= 6 ||
	...
	conversion2: {
		...
		onHit(target, source) {
			...
			for (let type in this.dex.data.TypeChart) {
				...
				let typeCheck = this.dex.data.TypeChart[type].damageTaken[
					attackType
				];
		...
		onModifyMove(move, pokemon, target) {
			if (target.side.active.length === 2) {
				...
				for (const currentTarget of target.side.active) {
					if (
						currentTarget.volatiles["protect"] ||
						!currentTarget.runImmunity("Dragon") ||
						currentTarget.fainted
					) {
						move.multihit = 2;
						break;
					}
	...
	electroball: {
		...
		basePowerCallback(pokemon, target) {
			...
			this.debug(
				[40, 60, 80, 120, 150][
					(Math.floor(ratio) > 4 ? 4 : Math.floor(ratio))
				] + " bp"
			);
			if (ratio >= 4) {
				return 150;
		...
		effect: {
			...
			onResidual(target) {
				if (
					...
					target.moveSlots[target.moves.indexOf(this.effectData.move)]
						.pp <= 0
				) {
			...
			onDisableMove(pokemon) {
				if (
					!this.effectData.move ||
					!pokemon.hasMove(this.effectData.move)
				) {
					return;
				}
				for (const moveSlot of pokemon.moveSlots) {
	...
	fusionbolt: {
		...
		onBasePower(basePower, pokemon) {
			if (
				this.lastMoveThisTurn &&
				this.lastMoveThisTurn.id === "fusionflare"
			) {
	...
	fusionflare: {
		...
		onBasePower(basePower, pokemon) {
			if (
				this.lastMoveThisTurn &&
				this.lastMoveThisTurn.id === "fusionbolt"
			) {
				this.debug("double power");
				return this.chainModify(2);
			}
	...
	gmaxbefuddle: {
		...
		category: "Physical",
		desc:
			"Sleeps, poisons, or paralyzes opponent(s). Base Power scales with the base move's Base Power.",
		...
		effect: {
			...
			onTerrain(pokemon) {
				if (pokemon.isGrounded() && !pokemon.isSemiInvulnerable()) {
					this.debug(
						"Pokemon is grounded, healing through Grassy Terrain."
					);
	...
	helpinghand: {
		...
		effect: {
			...
			onBasePower(basePower) {
				this.debug(
					"Boosting from Helping Hand: " + this.effectData.multiplier
				);
	...
	lastresort: {
		...
		flags: { contact: 1, protect: 1, mirror: 1 },
		onTryHit(target, source) {
			if (source.moveSlots.length < 2) return false; // Last Resort fails unless the user knows at least 2
			// moves
	...
	luckychant: {
		...
		effect: {
			duration: 5,
			onStart(side) {
				this.add("-sidestart", side, "move: Lucky Chant"); // "The Lucky Chant shielded [side.name]'s
			...
			onEnd(side) {
				this.add("-sideend", side, "move: Lucky Chant"); // "[side.name]'s team's Lucky Chant wore
				// off!"
			}
		},
		secondary: null,
	...
	lunardance: {
		...
		effect: {
			...
			onSwitchInPriority: 1,
			onSwitchIn(target) {
				const positions = /**@type {boolean[]} */ (this.effectData
					.positions);
		...
		effect: {
			...
			onTryMove(pokemon, target, move) {
				if (move.type === "Fire") {
					...
					this.damage(
						this.dex.clampIntRange(Math.round(pokemon.maxhp / 4), 1)
					);
					return false;
				}
			}
			...
			onBeforeSwitchOut(pokemon) {
				...
				for (const source of this.effectData.sources) {
					...
					this.runMove(
						"pursuit",
						source,
						this.getTargetLoc(pokemon, source)
					);
				}
			}
		},
	...
	rototiller: {
		...
		onHitField(target, source) {
			...
			if (!targets.length && !anyAirborne) return false; // Fails when there are no grounded Grass types or
			// airborne Pokemon
		...
		effect: {
			...
			onTryPrimaryHit(target, source, move) {
				...
				if (damage > target.volatiles["substitute"].hp) {
					damage =
						/** @type {number} */ (target.volatiles["substitute"].hp);
				}
