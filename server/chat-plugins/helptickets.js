 ...
 */
/**
 * @typedef {'approved' | 'valid' | 'assisted' | 'denied' | 'invalid' |
 * 'unassisted' | 'ticketban' | 'deleted'} TicketResult
 */
  ...
  for (let t in ticketData) {
    ...
    if (ticket.banned) {
      ...
      if (ticket.created + TICKET_CACHE_TIME <= Date.now()) {
        // Tickets that have been open for 24+ hours will be automatically
        // closed.
        const ticketRoom =
            /** @type {ChatRoom | null} */ (Rooms.get(`help-${ticket.userid}`));
        if (ticketRoom) {
      ...
      if (ticket.open && process.uptime() <= 60)
        ticket.open = false;
      tickets[t] = ticket;
    }
  }
...
class HelpTicket extends Rooms.RoomGame {
  ...
  onJoin(user, connection) {
    ...
    if (!this.ticket.claimed) {
      ...
      if (!this.firstClaimTime) {
        ...
        // I'd use the player list for this, but it dosen't track DCs so were
        // checking the userlist Non-staff users in the room currently (+ the
        // ticket creator even if they are staff)
        let users = Object.entries(this.room.users).filter(u => {
          return !((u[1].isStaff && u[1].id !== this.ticket.userid) ||
                   !u[1].named);
        });
        if (!users.length)
          this.emptyRoom = true;
      ...
      if (this.ticket.active) {
        ...
        this.lastUnclaimedStart =
            0; // Set back to 0 so we know that it was active when closed
      }
  ...
  onLeave(user, oldUserid) {
    ...
    if (toID(this.ticket.claimed) === user.id) {
      if (this.claimQueue.length) {
        ...
        this.modnote(user,
                     `This ticket is now claimed by ${this.ticket.claimed}.`);
      } else {
      ...
      let index =
          this.claimQueue.map(toID).indexOf(/** @type {ID} */ (user.id));
      if (index > -1)
  ...
  onLogMessage(message, user) {
    ...
    if ((!user.isStaff || this.ticket.userid === user.id) &&
        blockedMessages.includes(toID(message))) {
      this.room.add(
          `|c|~Staff|Hello! The global staff team would be happy to help you, but you need to explain what's going on first.`);
      this.room.add(
          `|c|~Staff|Please post the information I requested above so a global staff member can come to help.`);
      this.room.update();
      return false;
    }
    if ((!user.isStaff || this.ticket.userid === user.id) &&
      ...
      this.room
          .add(
              `|c|~Staff|Thank you for the information, global staff will be here shortly. Please stay in the room.`)
          .update();
    }
  }
  ...
  getPreview() {
    ...
    for (let i = this.room.log.log.length - 1; i >= 0; i--) {
      ...
      message = message.startsWith('/log ') ? message.slice(5)
                                            : `${user}: ${message}`;
  ...
  writeStats(result) {
    ...
    if (this.activationTime) {
      firstClaimWait =
          (this.firstClaimTime ? this.firstClaimTime : this.closeTime) -
          this.activationTime;
      involvedStaff =
          Array.from(this.involvedStaff.entries()).map(s => s[0]).join(',');
    }
    // Write to TSV
    // ticketType\ttotalTime\ttimeToFirstClaim\tinactiveTime\tresolution\tresult\tstaff,userids,seperated,with,commas
    const line = `${this.ticket.type}\t${(this.closeTime - this.createTime)}\t${
        firstClaimWait}\t${this.unclaimedTime}\t${this.resolution}\t${
        this.result}\t${involvedStaff}`;
    writeStats(line);
  }
  ...
  if (hasUnclaimed && !unclaimedTicketTimer[room.roomid]) {
    ...
    timerEnds[room.roomid] =
        ...
        (hasAssistRequest ? NOTIFY_ASSIST_TIMEOUT : NOTIFY_ALL_TIMEOUT);
             ...
             (timerEnds[room.roomid] - NOTIFY_ASSIST_TIMEOUT) >
    ...
    unclaimedTicketTimer[room.roomid] = setTimeout(
        () => notifyUnclaimedTicket(hasAssistRequest), NOTIFY_ASSIST_TIMEOUT);
...
function notifyUnclaimedTicket(hasAssistRequest) {
  ...
  for (let i in room.users) {
    ...
    if (user.can('mute', null, room) && !user.ignoreTickets)
      user.sendTo(room, `|tempnotify|helptickets|Unclaimed help tickets!|${
                            hasAssistRequest
                                ? 'Public Room Staff need help'
                                : 'There are unclaimed Help tickets'}`);
  }
}
...
function notifyStaff() {
  ...
  for (const key of keys) {
    ...
    const creator = ticket.claimed
                        ? Chat.html`${ticket.creator}`
                        : Chat.html`<strong>${ticket.creator}</strong>`;
    ...
    buf += `<a class="button${notifying}" href="/help-${ticket.userid}" ${
        ticketGame.getPreview()}>Help ${creator}: ${ticket.type}</a> `;
    count++;
  }
  if (hiddenTicketCount > 1) {
    ...
    buf = buf.slice(0, fourthTicketIndex) +
          `<a class="button${notifying}" href="/view-help-tickets">and ${
              hiddenTicketCount} more Help ticket${
              Chat.plural(hiddenTicketCount)} (${
              hiddenTicketUnclaimedCount} unclaimed)</a>`;
  }
  buf = `|${
      hasUnclaimed
          ? 'uhtml'
          : 'uhtmlchange'}|latest-tickets|<div class="infobox" style="padding: 6px 4px">${
      buf}${
      count === 0
          ? `There were open Help tickets, but they've all been closed now.`
          : ``}</div>`;
  room.send(buf);

  if (hasUnclaimed) {
    buf = `|tempnotify|helptickets|Unclaimed help tickets!|${
        hasAssistRequest ? 'Public Room Staff need help'
                         : 'There are unclaimed Help tickets'}`;
  } else {
    buf = `|tempnotifyoff|helptickets`;
  }
  if (room.userCount)
    Sockets.roomBroadcast(room.roomid, `>view-help-tickets\n${buf}`);
  if (hasUnclaimed) {
...
function checkIp(ip) {
  for (let t in tickets) {
    if (tickets[t].ip === ip && tickets[t].open &&
        !Punishments.sharedIps.has(ip)) {
      return tickets[t];
    }
  }
  return false;
...
function checkTicketBanned(user) {
  ...
  if (ticket) {
    ...
    const checkIp =
        !(Punishments.sharedIps.has(user.latestIp) && user.autoconfirmed);
    if (checkIp) {
...
const pages = {
  help : {
    request(query, user, connection) {
      ...
      if (banMsg)
        return connection.popup(banMsg);
      ...
      if ((ticket && ticket.open) || ipTicket) {
        ...
        if (!helpRoom) {
          ...
          if (!helpRoom.auth[user.id])
            helpRoom.auth[user.id] = '+';
      ...
      let meta = '';
      const targetTypeIndex =
          Math.max(query.indexOf('user'), query.indexOf('room'));
      ...
      for (const [i, page] of query.entries()) {
        ...
        if (page && page in ticketPages && !page.startsWith('confirm')) {
          ...
          buf += `<p><a href="/view-help-request${prevPageLink}${
              !isFirst
                  ? meta
                  : ''}" target="replace"><button class="button">Back</button></a> <button class="button disabled" disabled>${
              ticketPages[page]}</button></p>`;
        }
        switch (page) {
        case '':
          ...
          if (isStaff) {
            buf +=
                `<p class="message-error">Global staff cannot make Help requests. This form is only for reference.</p>`;
          } else {
            buf +=
                `<p class="message-error">Abuse of Help requests can result in punishments.</p>`;
          }
          if (!isLast)
            break;
        ...
        case 'pmharassment':
          buf +=
              `<p>If someone is harrassing you in private messages (PMs), click the button below and a global staff member will take a look. If you are being harassed in a chatroom, please ask a room staff member to handle it. If it's a minor issue, consider using <code>/ignore [username]</code> instead.</p>`;
          if (!isLast)
            break;
        ...
        case 'battleharassment':
          buf +=
              `<p>If someone is harrassing you in a battle, click the button below and a global staff member will take a look. If you are being harassed in a chatroom, please ask a room staff member to handle it. If it's a minor issue, consider using <code>/ignore [username]</code> instead.</p>`;
          buf +=
              `<p>Please save a replay of the battle if it has ended, or provide a link to the battle if it is still ongoing.</p>`;
          if (!isLast)
            break;
        ...
        case 'inapname':
          buf +=
              `<p>If a user has an inappropriate name or status message, click the button below and a global staff member will take a look.</p>`;
          if (!isLast)
            break;
        ...
        case 'inappokemon':
          buf +=
              `<p>If a user has inappropriate Pokemon nicknames, click the button below and a global staff member will take a look.</p>`;
          buf +=
              `<p>Please save a replay of the battle if it has ended, or provide a link to the battle if it is still ongoing.</p>`;
          if (!isLast)
            break;
        ...
        case 'permalock':
          buf +=
              `<p>Permalocks are usually for repeated incidents of poor behavior over an extended period of time, and rarely for a single severe infraction. Please keep this in mind when appealing a permalock.</p>`;
          buf +=
              `<p>Please visit the <a href="https://www.smogon.com/forums/threads/discipline-appeal-rules.3583479/">Discipline Appeals</a> page to appeal your permalock.</p>`;
          break;
        case 'lock':
          buf +=
              `<p>If you want to appeal your lock or namelock, click the button below and a global staff member will be with you shortly.</p>`;
          if (!isLast)
            break;
        ...
        case 'ip':
          buf +=
              `<p>If you are locked or namelocked under a name you don't recognize, click the button below to call a global staff member so we can check.</p>`;
          if (!isLast)
            break;
        ...
        case 'hostfilter':
          buf +=
              `<p>We automatically lock proxies and VPNs to prevent evasion of punishments and other attacks on our server. To get unlocked, you need to disable your proxy or VPN.</p>`;
          break;
        case 'semilock':
          buf +=
              `<p>Do you have an autoconfirmed account? An account is autoconfirmed when it has won at least one rated battle and has been registered for one week or longer.</p>`;
          if (!isLast)
            break;
          buf +=
              `<p><Button>hasautoconfirmed</Button> <Button>lacksautoconfirmed</Button></p>`;
          break;
        case 'hasautoconfirmed':
          buf +=
              `<p>Login to your autoconfirmed account by using the <code>/nick</code> command in any chatroom, and the semilock will automatically be removed. Afterwords, you can use the <code>/nick</code> command to switch back to your current username without being semilocked again.</p>`;
          buf +=
              `<p>If the semilock does not go away, you can try asking a global staff member for help. Click the button below to call a global staff member.</p>`;
          if (!isLast)
            break;
        ...
        case 'lacksautoconfirmed':
          buf +=
              `<p>If you don't have an autoconfirmed account, you will need to contact a global staff member to appeal your semilock. Click the button below to call a global staff member.</p>`;
          if (!isLast)
            break;
          buf += `<p><Button>confirmappealsemi</Button></p>`;
          break;
        case 'appealother':
          buf +=
              `<p>Please PM the staff member who punished you. If you don't know who punished you, ask another room staff member; they will redirect you to the correct user. If you are banned or blacklisted from the room, use <code>/roomauth [name of room]</code> to get a list of room staff members. Bold names are online.</p>`;
          break;
        case 'misc':
        ...
        case 'password':
          buf +=
              `<p>If you lost your password, click the button below to request a password reset. We will need to clarify a few pieces of information before resetting the account. Please note that password resets are low priority and may take a while; we recommend using a new account while waiting.</p>`;
          buf +=
              `<p><a class="button" href="https://www.smogon.com/forums/password-reset-form/">Request a password reset</a></p>`;
          break;
        case 'roomhelp':
          buf +=
              `<p>If you are a room driver or up in a public room, and you need help watching the chat, one or more global staff members would be happy to assist you!</p>`;
          buf += `<p><Button>confirmroomhelp</Button></p>`;
          break;
        case 'other':
          buf +=
              `<p>If your issue is not handled above, click the button below to talk to a global staff member. Please be ready to explain the situation.</p>`;
          if (!isLast)
            break;
        ...
        default:
          if (!page.startsWith('confirm') || !ticketTitles[page.slice(7)]) {
            ...
            buf +=
                `<a href="/view-help-request" target="replace"><button class="button">Back</button></a>`;
            break;
          }
          buf += `<p><b>Are you sure you want to submit a${
              ticketTitles[page.slice(7)].charAt(0) === 'A'
                  ? 'n'
                  : ''} ${ticketTitles[page.slice(7)]} report?</b></p>`;
          const submitMeta = Chat.splitFirst(meta, '-', 2)
                                 .join('|'); // change the delimiter as some
                                             // ticket titles include -
          buf += `<p><button class="button notifying" name="send" value="/helpticket submit ${
              ticketTitles[page.slice(7)]} ${
              submitMeta}">Yes, Contact global staff</button> <a href="/view-help-request-${
              query.slice(0, i).join('-')}${
              meta}" target="replace"><button class="button">No, cancel</button></a></p>`;
          break;
        }
      }
      buf += '</div>';
      ...
      buf = buf.replace(
          /<Button>([a-z]+)<\/Button>/g,
          (match, id) =>
              `<a class="button" href="/view-help-request${curPageLink}-${id}${
                  meta}" target="replace">${ticketPages[id]}</a>`);
      return buf;
    },
    tickets(query, user, connection) {
      ...
      if (!this.can('lock'))
        return;
      let buf =
          `<div class="pad ladder"><button class="button" name="send" value="/helpticket list" style="float:left"><i class="fa fa-refresh"></i> Refresh</button> <button class="button" name="send" value="/helpticket stats" style="float: right"><i class="fa fa-th-list"></i> Help Ticket Stats</button><br /><br />`;
      buf +=
          `<table style="margin-left: auto; margin-right: auto"><tbody><tr><th colspan="5"><h2 style="margin: 5px auto">Help tickets</h1></th></tr>`;
      buf +=
          `<tr><th>Status</th><th>Creator</th><th>Ticket Type</th><th>Claimed by</th><th>Action</th></tr>`;
      ...
      for (const key of keys) {
        if (count >= 100 && query[0] !== 'all') {
          buf += `<tr><td colspan="5">And ${
              keys.length -
              count} more tickets. <a class="button" href="/view-help-tickets-all" target="replace">View all tickets</a></td></tr>`;
          break;
        }
        const ticket = tickets[key];
        let icon =
            `<span style="color:gray"><i class="fa fa-check-circle-o"></i> Closed</span>`;
        if (ticket.open) {
          if (!ticket.active) {
            icon =
                `<span style="color:gray"><i class="fa fa-circle-o"></i> Inactive</span>`;
          } else if (ticket.claimed) {
            icon =
                `<span style="color:green"><i class="fa fa-circle-o"></i> Claimed</span>`;
          } else {
            icon =
                `<span style="color:orange"><i class="fa fa-circle-o"></i> <strong>Unclaimed</strong></span>`;
          }
        }
        buf += `<tr><td>${icon}</td>`;
        ...
        if (room) {
          ...
          buf += `<a href="/${roomid}"><button class="button" ${
              ticketGame.getPreview()}>${
              !ticket.claimed && ticket.open ? 'Claim' : 'View'}</button></a> `;
        }
        if (logUrl) {
      ...
      for (const key of banKeys) {
        ...
        if (!hasBanHeader) {
          buf +=
              `<tr><th>Status</th><th>Username</th><th>Banned by</th><th>Expires</th><th>Logs</th></tr>`;
          hasBanHeader = true;
        }
        if (count >= 100 && query[0] !== 'all') {
          buf += `<tr><td colspan="5">And ${
              banKeys.length -
              count} more ticket bans. <a class="button" href="/view-help-tickets-all" target="replace">View all tickets</a></td></tr>`;
          break;
        }
        buf +=
            `<tr><td><span style="color:gray"><i class="fa fa-ban"></i> Banned</td>`;
        buf += Chat.html`<td>${ticket.name}</td>`;
        buf += Chat.html`<td>${ticket.by}</td>`;
        buf += `<td>${Chat.toDurationString(ticket.expires - Date.now(), {
          precision : 1
        })}</td>`;
        buf += `<td>`;
        const roomid = 'help-' + ticket.userid;
        let logUrl = '';
        if (Config.modloglink) {
          const modlogDate = new Date(
              ticket.created ||
              (ticket.banned ? ticket.expires - TICKET_BAN_DURATION : 0));
    ...
    stats(query, user, connection) {
      ...
      if (monthString.includes('undefined')) {
        ...
        date = new Date();
        monthString =
            Chat.toTimestamp(date).split(' ')[0].split('-', 2).join('-');
      }
      let rawTicketStats =
          FS(`logs/tickets/${monthString}.tsv`).readIfExistsSync();
      if (!rawTicketStats)
        return `<div class="pad"><br />No ticket stats found.</div>`;

      // Calculate next/previous month for stats and validate stats exist for
      // the month
      const prevDate = new Date(monthString);
      if (!date)
        date = new Date(
            (prevDate.getMonth() === 11 ? prevDate.getFullYear() + 1
                                        : prevDate.getFullYear()),
            (prevDate.getMonth() === 11 ? 0 : prevDate.getMonth() + 1));
      const nextDate = new Date(
          (prevDate.getMonth() >= 10 ? prevDate.getFullYear() + 1
                                     : prevDate.getFullYear()),
          (prevDate.getMonth() + 2 > 11 ? (prevDate.getMonth() + 2) - 12
                                        : prevDate.getMonth() + 2));
      let prevString =
          Chat.toTimestamp(prevDate).split(' ')[0].split('-', 2).join('-');
      let nextString =
          Chat.toTimestamp(nextDate).split(' ')[0].split('-', 2).join('-');

      let buttonBar = '';
      if (FS(`logs/tickets/${prevString}.tsv`).readIfExistsSync()) {
        buttonBar += `<a class="button" href="/view-help-stats-${table}-${
            prevString}" target="replace" style="float: left">&lt; Previous Month</a>`;
      } else {
        buttonBar +=
            `<a class="button disabled" style="float: left">&lt; Previous Month</a>`;
      }
      buttonBar += `<a class="button${
          table === 'tickets'
              ? ' disabled"'
              : `" href="/view-help-stats-tickets-${
                    monthString}" target="replace"`}>Ticket Stats</a> <a class="button ${
          table === 'staff'
              ? ' disabled"'
              : `" href="/view-help-stats-staff-${
                    monthString}" target="replace"`}>Staff Stats</a>`;
      if (FS(`logs/tickets/${nextString}.tsv`).readIfExistsSync()) {
        buttonBar += `<a class="button" href="/view-help-stats-${table}-${
            nextString}" target="replace" style="float: right">Next Month &gt;</a>`;
      } else {
        buttonBar +=
            `<a class="button disabled" style="float: right">Next Month &gt;</a>`;
      }

      let buf = `<div class="pad ladder"><div style="text-align: center">${
          buttonBar}</div><br />`;
      buf +=
          `<table style="margin-left: auto; margin-right: auto"><tbody><tr><th colspan="${
              table === 'tickets'
                  ? 7
                  : 3}"><h2 style="margin: 5px auto">Help Ticket Stats - ${
              date.toLocaleString(
                  'en-us', {month : 'long', year : 'numeric'})}</h1></th></tr>`;
      if (table === 'tickets') {
        if (!['type', 'totaltickets', 'total', 'initwait', 'wait', 'resolution',
              'result']
                 .includes(col))
          col = 'type';
        buf +=
            `<tr><th><Button>type</Button></th><th><Button>totaltickets</Button></th><th><Button>total</Button></th><th><Button>initwait</Button></th><th><Button>wait</Button></th><th><Button>resolution</Button></th><th><Button>result</Button></th></tr>`;
      } else {
        if (!['staff', 'num', 'time'].includes(col))
          col = 'num';
        buf +=
            `<tr><th><Button>staff</Button></th><th><Button>num</Button></th><th><Button>time</Button></th></tr>`;
      }

      /** @type {{[key: string]: string}[]} */
      let ticketStats =
          rawTicketStats.split('\n').filter(line => line).map(line => {
      ...
      if (table === 'tickets') {
        ...
        for (let stats of ticketStats) {
          ...
          if ([ 'approved', 'valid', 'assisted' ].includes(
                  stats.result.toString()))
            type.result++;
          if ([ 'dead', 'unresolved', 'resolved' ].includes(
                  stats.resolution.toString())) {
        ...
        for (let type of sortedStats) {
          const resolution = `Resolved: ${
              typeStats[type].resolved}%<br/>Unresolved: ${
              typeStats[type].unresolved}%<br/>Dead: ${typeStats[type].dead}%`;
          buf += `<tr><td>${type}</td><td>${
              typeStats[type].totaltickets}</td><td>${
              Chat.toDurationString(typeStats[type].total, {
                hhmmss : true
              })}</td><td>${Chat.toDurationString(typeStats[type].initwait, {
            hhmmss : true
          }) || '-'}</td><td>${Chat.toDurationString(typeStats[type].wait, {
            hhmmss : true
          }) || '-'}</td><td>${resolution}</td><td>${
              typeStats[type].result}%</td></tr>`;
        }
      } else {
        /** @type {{[key: string]: {[key: string]: number}}} */
        let staffStats = {};
        for (let stats of ticketStats) {
          let staff =
              (typeof stats.staff === 'string' ? stats.staff.split(',') : []);
          for (let s = 0; s < staff.length; s++) {
            if (!staff[s])
              continue;
            if (!staffStats[staff[s]])
              staffStats[staff[s]] = {num : 0, time : 0};
            staffStats[staff[s]].num++;
            staffStats[staff[s]].time +=
                (parseInt(stats.total) - parseInt(stats.initwait));
          }
        }
        for (let staff in staffStats) {
          staffStats[staff].time =
              Math.round(staffStats[staff].time / staffStats[staff].num);
        }
        ...
        for (let staff of sortedStaff) {
          buf += `<tr><td>${staff}</td><td>${staffStats[staff].num}</td><td>${
              Chat.toDurationString(staffStats[staff].time,
                                    {precision : 1})}</td></tr>`;
        }
      }
      buf += `</tbody></table></div>`;
      ...
      buf = buf.replace(/<Button>([a-z]+)<\/Button>/g, (match, id) => {
        if (col === id)
          return headerTitles[id];
        return `<a class="button" href="/view-help-stats-${table}-${
            monthString}-${id}" target="replace">${headerTitles[id]}</a>`;
      });
      return buf;
    },
  },
...
let commands = {
  ...
  report(target, room, user) {
    if (!this.runBroadcast())
      return;
    const meta = this.pmTarget ? `-user-${this.pmTarget.id}`
                               : this.room ? `-room-${this.room.roomid}` : '';
    if (this.broadcasting) {
      if (room && room.battle)
        return this.errorReply(`This command cannot be broadcast in battles.`);
      return this.sendReplyBox(
          `<button name="joinRoom" value="view-help-request--report${
              meta}" class="button"><strong>Report someone</strong></button>`);
    }
  ...
  appeal(target, room, user) {
    ...
    if (this.broadcasting) {
      if (room && room.battle)
        return this.errorReply(`This command cannot be broadcast in battles.`);
      return this.sendReplyBox(
          `<button name="joinRoom" value="view-help-request--appeal${
              meta}" class="button"><strong>Appeal a punishment</strong></button>`);
    }
  ...
  helpticket : {
    ...
    create(target, room, user) {
      ...
      if (this.broadcasting) {
        return this.sendReplyBox(
            `<button name="joinRoom" value="view-help-request${
                meta}" class="button"><strong>Request help</strong></button>`);
      }
      if (user.can('lock'))
        return this.parse(
            '/join view-help-request'); // Globals automatically get the form
                                        // for reference.
      if (!user.named)
        return this.errorReply(
            `You need to choose a username before doing this.`);
    ...
    submit(target, room, user, connection) {
      ...
      if (Monitor.countTickets(user.latestIp))
        return this.popupReply(`Due to high load, you are limited to creating ${
            Punishments.sharedIps.has(user.latestIp)
                ? `50`
                : `5`} tickets every hour.`);
      let [ticketType, reportTargetType, reportTarget] =
          Chat.splitFirst(target, '|', 2).map(s => s.trim());
      ...
      const contexts = {
        ...
        'Inappropriate Username / Status Message' :
            `Hi! Tell us the username that is inappropriate, or tell us which user has an inappropriate status message.`,
        ...
        'Other' :
            `Hi! What seems to be the problem? Tell us about any people involved, and if this happened in a specific place on the site.`,
      };
      /** @type {{[k: string]: string}} */
      const staffContexts = {
        'IP-Appeal' : `<p><strong>${user.name}'s IP Addresses</strong>: ${
            Object.keys(user.ips)
                .map(ip => `<a href="https://whatismyipaddress.com/ip/${
                         ip}" target="_blank">${ip}</a>`)
                .join(', ')}</p>`,
      };
      ticket = {
        creator : user.name,
      ...
      case 'ISP-Appeal':
        closeButtons =
            `<button class="button" style="margin: 5px 0" name="send" value="/helpticket close ${
                user.id}">Close Ticket as Appeal Granted</button> <button class="button" style="margin: 5px 0" name="send" value="/helpticket close ${
                user.id}, false">Close Ticket as Appeal Denied</button>`;
        break;
      case 'PM Harassment':
      case 'Battle Harassment':
      case 'Inappropriate Pokemon Nicknames':
      case 'Inappropriate Username / Status Message':
        closeButtons =
            `<button class="button" style="margin: 5px 0" name="send" value="/helpticket close ${
                user.id}">Close Ticket as Valid Report</button> <button class="button" style="margin: 5px 0" name="send" value="/helpticket close ${
                user.id}, false">Close Ticket as Invalid Report</button>`;
        break;
      case 'Public Room Assistance Request':
      case 'Other':
      default:
        closeButtons =
            `<button class="button" style="margin: 5px 0" name="send" value="/helpticket close ${
                user.id}">Close Ticket as Assisted</button> <button class="button" style="margin: 5px 0" name="send" value="/helpticket close ${
                user.id}, false">Close Ticket as Unable to Assist</button>`;
      }
      ...
      if (reportTargetType === 'user' && reportTarget) {
        ...
        case 'PM Harassment':
          ...
          contexts['PM Harassment'] =
              `Hi! Please click the button below to give global staff permission to check PMs. Or if ${
                  reportTarget} is not the user you want to report, please tell us the name of the user who you want to report.`;
          break;
        case 'Inappropriate Username / Status Message':
          staffIntroButtons =
              `<button class="button" name="send" value="/forcerename ${
                  reportTarget}">Force-rename ${
                  reportTarget}</button> <button class="button" name="send" value="/clearstatus ${
                  reportTarget}">Clear ${reportTarget}'s status</button> `;
          break;
        case 'Battle Harassment':
        case 'Inappropriate Pokemon Nicknames':
          staffIntroButtons =
              `<button class="button" name="send" value="/sharedbattles ${
                  user.id}, ${toID(reportTarget)}">Shared battles</button> `;
          break;
        }
        staffIntroButtons +=
            `<button class="button" name="send" value="/modlog global, ${
                reportTarget}">Global Modlog for ${reportTarget}</button> `;
      }
      const introMessage = Chat.html`<h2 style="margin-top:0">Help Ticket - ${
          user.name}</h2><p><b>Issue</b>: ${
          ticket
              .type}<br />A Global Staff member will be with you shortly.</p>`;
      const staffMessage = `<p>${
          closeButtons} <details><summary class="button">More Options</summary> ${
          staffIntroButtons}<button class="button" name="send" value="/helpticket ban ${
          user.id}"><small>Ticketban</small></button></details></p>`;
      const staffHint = staffContexts[ticketType] || '';
      const reportTargetInfo =
          reportTargetType === 'room'
              ? `Reported in room: <a href="/${reportTarget}">${
                    reportTarget}</a>`
              : reportTargetType === 'user'
                    ? `Reported user: <strong class="username">${
                          reportTarget}</strong>`
                    : '';
      let helpRoom = /** @type {ChatRoom?} */ (Rooms.get(`help-${user.id}`));
      if (!helpRoom) {
        helpRoom = Rooms.createChatRoom(
            /** @type {RoomID} */ (`help-${user.id}`), `[H] ${user.name}`, {
              isPersonal : true,
              isHelp : 'open',
              isPrivate : 'hidden',
              modjoin : '%',
        ...
        if (helpRoom.expireTimer)
          clearTimeout(helpRoom.expireTimer);
      ...
      ticketGame.modnote(
          user, `${user.name} opened a new ticket. Issue: ${ticket.type}`);
      this.parse(`/join help-${user.id}`);
      if (!(user.id in ticketGame.playerTable)) {
        // User was already in the room, manually add them to the "game" so they
        // get a popup if they try to leave
    ...
    statshelp : [
      `/helpticket stats - List the stats for help tickets. Requires: % @ & ~`
    ],
    ...
    close(target, room, user) {
      ...
      if (!ticket || !ticket.open ||
          (ticket.userid !== user.id && !user.can('lock')))
        return this.errorReply(
            `${this.inputUsername} does not have an open ticket.`);
      const helpRoom =
          /** @type {ChatRoom?} */ (Rooms.get(`help-${ticket.userid}`));
      if (helpRoom) {
        const ticketGame = /** @type {HelpTicket} */ (helpRoom.game);
    ...
    ban(target, room, user) {
      ...
      if (!this.can('lock', targetUser))
        return;

      let ticket = tickets[toID(this.inputUsername)];
      let ticketBan = ticketBans[toID(this.inputUsername)];
      if (!targetUser &&
          !Punishments.search(toID(this.targetUsername)).length && !ticket &&
          !ticketBan) {
      ...
      if (targetUser) {
        ...
        if (ticketBan && ticketBan.expires > Date.now())
          return this.privateModAction(`(${name} would be ticket banned by ${
              user.name} but was already ticket banned.)`);
        if (targetUser.trusted)
          Monitor.log(`[CrisisMonitor] Trusted user ${targetUser.name}${
              (targetUser.trusted !== targetUser.id
                   ? ` (${targetUser.trusted})`
                   : ``)} was ticket banned by ${
              user.name}, and should probably be demoted.`);
      } else {
      ...
      if (targetUser) {
        targetUser.popup(
            `|modal|${user.name} has banned you from creating help tickets.${
                (target ? `\n\nReason: ${target}`
                        : ``)}\n\nYour ban will expire in a few days.`);
      }

      this.addModAction(`${name} was ticket banned by ${user.name}.${
          target ? ` (${target})` : ``}`);
      ...
      let punishment = /** @type {BannedTicketState} */ ({
        ...
        reason : target,
        ip : (targetUser ? targetUser.latestIp
                         : ticket ? ticket.ip : ticketBan.ip),
      });
      ...
      if (targetUser) {
        ...
        affected =
            Users.findUsers(/** @type {ID[]} */ ([...userids ]), [...ips ],
                            {includeTrusted : true, forPunishment : true});
        affected.unshift(userid);
      }

      let acAccount = (targetUser && targetUser.autoconfirmed !== userid &&
      ...
      if (affected.length > 1) {
        displayMessage = `(${name}'s ${
            acAccount ? ` ac account: ${acAccount}, `
                      : ""}ticket banned alts: ${
            affected.slice(1).map(user => user.getLastName()).join(", ")})`;
      ...
      for (let user of affected) {
        ...
        if (targetTicket && targetTicket.open)
      ...
      this.globalModlog(`TICKETBAN`, targetUser || userid,
                        ` by ${user.name}${(target ? `: ${target}` : ``)}`);
      return true;
    },
    banhelp : [
    ...
    unban(target, room, user) {
      ...
      if (ticket.expires <= Date.now()) {
        ...
        return this.errorReply(`${
            targetUser ? targetUser.name
                       : target}'s ticket ban is already expired.`);
      }

      let affected = [];
      for (let t in ticketBans) {
        if (toID(ticketBans[t].banned) === toID(ticket.banned) &&
            ticketBans[t].userid !== ticket.userid) {
      ...
      this.addModAction(`${affected.join(', ')} ${
          Chat.plural(affected.length, "were",
                      "was")} ticket unbanned by ${user.name}.`);
      this.globalModlog("UNTICKETBAN", target, ` by ${user.id}`);
      if (targetUser)
        targetUser.popup(`${user.name} has ticket unbanned you.`);
    },
    unbanhelp : [
    ...
    deletehelp :
        [ `/helpticket delete [user] - Deletes a users ticket. Requires: & ~` ],
