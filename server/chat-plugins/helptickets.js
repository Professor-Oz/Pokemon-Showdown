	...
	for (let t in ticketData) {
		...
		if (ticket.banned) {
			...
			if (ticket.created + TICKET_CACHE_TIME <= Date.now()) {
				...
				// closed.
				const ticketRoom = /** @type {ChatRoom | null} */ (Rooms.get(
					`help-${ticket.userid}`
				));
				if (ticketRoom) {
...
function writeStats(line) {
	...
	const date = new Date();
	const month = Chat.toTimestamp(date)
		.split(" ")[0]
		.split("-", 2)
		.join("-");
	try {
...
class HelpTicket extends Rooms.RoomGame {
		...
		if (!this.ticket.claimed) {
			...
			if (!this.firstClaimTime) {
				...
				let users = Object.entries(this.room.users).filter(u => {
					return !(
						(u[1].isStaff && u[1].id !== this.ticket.userid) ||
						!u[1].named
					);
				});
	...
	onLeave(user, oldUserid) {
		...
		if (toID(this.ticket.claimed) === user.id) {
			...
			let index = this.claimQueue
				.map(toID)
				.indexOf(/** @type {ID} */ (user.id));
	...
	forfeit(user) {
		...
		this.modnote(
			user,
			`${user.name} is no longer interested in this ticket.`
		);
	...
	getPreview() {
		...
		for (let i = this.room.log.log.length - 1; i >= 0; i--) {
			...
			message = message.startsWith("/log ")
				? message.slice(5)
				: `${user}: ${message}`;
	...
	writeStats(result) {
		...
		// Write to TSV
		// ticketType\ttotalTime\ttimeToFirstClaim\tinactiveTime\tresolution\tresult\tstaff,userids,seperated,with,commas
		const line = `${this.ticket.type}\t${this.closeTime -
			this.createTime}\t${firstClaimWait}\t${this.unclaimedTime}\t${
			this.resolution
...
function notifyUnclaimedTicket(hasAssistRequest) {
	...
	for (let i in room.users) {
		...
		if (user.can("mute", null, room) && !user.ignoreTickets)
			user.sendTo(
				room,
				`|tempnotify|helptickets|Unclaimed help tickets!|${
					hasAssistRequest
...
function notifyStaff() {
	...
	for (const key of keys) {
		...
		// should always exist
		const ticketRoom = /** @type {Room} */ (Rooms.get(
			`help-${ticket.userid}`
		));
		const ticketGame = /** @type {HelpTicket} */ (ticketRoom.game);
		if (!ticket.claimed) {
		...
		buf += `<a class="button${notifying}" href="/help-${
			ticket.userid
	...
	if (hiddenTicketCount > 1) {
		...
		buf =
			...
			`<a class="button${notifying}" href="/view-help-tickets">and ${hiddenTicketCount} more Help ticket${Chat.plural(
				hiddenTicketCount
			)} (${hiddenTicketUnclaimedCount} unclaimed)</a>`;
	}
	buf = `|${
		hasUnclaimed ? "uhtml" : "uhtmlchange"
	}|latest-tickets|<div class="infobox" style="padding: 6px 4px">${buf}${
		count === 0
	...
	help: {
				...
				switch (page) {
					...
					case "appeal":
						...
						if (user.locked || isStaff) {
							const namelocked =
								user.named && user.id.startsWith("guest");
					...
					default:
						...
						buf += `<p><b>Are you sure you want to submit a${
							ticketTitles[page.slice(7)].charAt(0) === "A" ? "n" : ""
						} ${ticketTitles[page.slice(7)]} report?</b></p>`;
						const submitMeta = Chat.splitFirst(meta, "-", 2).join("|"); // change the delimiter as some
						// ticket titles include -
						buf += `<p><button class="button notifying" name="send" value="/helpticket submit ${
							ticketTitles[page.slice(7)]
						} ${submitMeta}">Yes, Contact global staff</button> <a href="/view-help-request-${query
							.slice(0, i)
							.join(
								"-"
		...
		tickets(query, user, connection) {
			...
			for (const key of keys) {
				if (count >= 100 && query[0] !== "all") {
					buf += `<tr><td colspan="5">And ${keys.length -
				...
				if (room) {
					...
					buf += `<a href="/${roomid}"><button class="button" ${ticketGame.getPreview()}>${
						!ticket.claimed && ticket.open ? "Claim" : "View"
					}</button></a> `;
				}
				if (logUrl) {
			...
			for (const key of banKeys) {
				...
				if (count >= 100 && query[0] !== "all") {
					buf += `<tr><td colspan="5">And ${banKeys.length -
		...
		stats(query, user, connection) {
			...
			if (monthString.includes("undefined")) {
				...
				date = new Date();
				monthString = Chat.toTimestamp(date)
					.split(" ")[0]
					.split("-", 2)
					.join("-");
			}
			let rawTicketStats = FS(
				`logs/tickets/${monthString}.tsv`
			).readIfExistsSync();
			if (!rawTicketStats)
			...
			const nextDate = new Date(
				(prevDate.getMonth() >= 10
					? prevDate.getFullYear() + 1
					: prevDate.getFullYear()),
				(prevDate.getMonth() + 2 > 11
					? prevDate.getMonth() + 2 - 12
					: prevDate.getMonth() + 2)
			);
			let prevString = Chat.toTimestamp(prevDate)
				.split(" ")[0]
				.split("-", 2)
				.join("-");
			let nextString = Chat.toTimestamp(nextDate)
				.split(" ")[0]
				.split("-", 2)
				.join("-");
			...
			buttonBar += `<a class="button${
				table === "tickets"
					? ' disabled"'
					: `" href="/view-help-stats-tickets-${monthString}" target="replace"`
			}>Ticket Stats</a> <a class="button ${
				table === "staff"
					? ' disabled"'
					: `" href="/view-help-stats-staff-${monthString}" target="replace"`
			}>Staff Stats</a>`;
			...
			buf += `<table style="margin-left: auto; margin-right: auto"><tbody><tr><th colspan="${
				table === "tickets" ? 7 : 3
			}"><h2 style="margin: 5px auto">Help Ticket Stats - ${date.toLocaleString(
				"en-us",
			...
			let ticketStats = rawTicketStats
				.split("\n")
				.filter(line => line)
				.map(line => {
			...
			if (table === "tickets") {
				...
				for (let stats of ticketStats) {
					...
					if (
						["approved", "valid", "assisted"].includes(
							stats.result.toString()
						)
					)
						type.result++;
					if (
						["dead", "unresolved", "resolved"].includes(
				...
				let sortedStats = Object.keys(typeStats).sort((a, b) => {
					if (col === "type") {
						...
						return (
							(typeStats[b].resolved || 0) - (typeStats[a].resolved || 0)
						);
					}
				...
				for (let type of sortedStats) {
					...
					buf += `<tr><td>${type}</td><td>${
						typeStats[type].totaltickets
					}</td><td>${Chat.toDurationString(typeStats[type].total, {
						hhmmss: true
					})}</td><td>${Chat.toDurationString(typeStats[type].initwait, {
						hhmmss: true
					}) || "-"}</td><td>${Chat.toDurationString(
						typeStats[type].wait,
						{
							hhmmss: true
						}
					) || "-"}</td><td>${resolution}</td><td>${
						typeStats[type].result
					}%</td></tr>`;
				}
			} else {
				/** @type {{[key: string]: {[key: string]: number}}} */
				...
				for (let staff in staffStats) {
					staffStats[staff].time = Math.round(
						staffStats[staff].time / staffStats[staff].num
					);
				}
				...
				for (let staff of sortedStaff) {
					buf += `<tr><td>${staff}</td><td>${
						staffStats[staff].num
					}</td><td>${Chat.toDurationString(staffStats[staff].time, {
						precision: 1
					})}</td></tr>`;
				}
			}
...
let commands = {
	...
	report(target, room, user) {
		...
		if (this.broadcasting) {
			...
			return this.sendReplyBox(
				`<button name="joinRoom" value="view-help-request--report${meta}" class="button"><strong>Report someone</strong></button>`
			);
		}
	...
	appeal(target, room, user) {
		...
		if (this.broadcasting) {
			...
			return this.sendReplyBox(
				`<button name="joinRoom" value="view-help-request--appeal${meta}" class="button"><strong>Appeal a punishment</strong></button>`
			);
		}
	...
	helpticket: {
		...
		create(target, room, user) {
			...
			if (this.broadcasting) {
				return this.sendReplyBox(
					`<button name="joinRoom" value="view-help-request${meta}" class="button"><strong>Request help</strong></button>`
				);
			}
			if (user.can("lock")) return this.parse("/join view-help-request"); // Globals automatically get the form
			// for reference.
			if (!user.named)
				return this.errorReply(
		...
		submit(target, room, user, connection) {
			...
			if (Monitor.countTickets(user.latestIp))
				return this.popupReply(
					`Due to high load, you are limited to creating ${
						Punishments.sharedIps.has(user.latestIp) ? `50` : `5`
					} tickets every hour.`
				);
			let [ticketType, reportTargetType, reportTarget] = Chat.splitFirst(
				target,
				"|",
				2
			).map(s => s.trim());
			...
			const staffContexts = {
				"IP-Appeal": `<p><strong>${
					user.name
				}'s IP Addresses</strong>: ${Object.keys(user.ips)
					.map(
						ip =>
			...
			if (reportTargetType === "user" && reportTarget) {
				switch (ticket.type) {
					case "PM Harassment":
						...
						pmRequestButton = Config.pmLogButton(
							user.id,
							toID(reportTarget)
						);
						contexts[
							"PM Harassment"
						] = `Hi! Please click the button below to give global staff permission to check PMs. Or if ${reportTarget} is not the user you want to report, please tell us the name of the user who you want to report.`;
						break;
					case "Inappropriate Username / Status Message":
					...
					case "Battle Harassment":
					case "Inappropriate Pokemon Nicknames":
						staffIntroButtons = `<button class="button" name="send" value="/sharedbattles ${
							user.id
						}, ${toID(reportTarget)}">Shared battles</button> `;
						break;
				}
			...
			const reportTargetInfo =
				reportTargetType === "room"
					? `Reported in room: <a href="/${reportTarget}">${reportTarget}</a>`
					: reportTargetType === "user"
					? `Reported user: <strong class="username">${reportTarget}</strong>`
					: "";
			let helpRoom = /** @type {ChatRoom?} */ (Rooms.get(`help-${user.id}`));
			if (!helpRoom) {
				helpRoom = Rooms.createChatRoom(
					/** @type {RoomID} */ (`help-${user.id}`),
					`[H] ${user.name}`,
					{
						isPersonal: true,
						isHelp: "open",
			...
			ticketGame.modnote(
				user,
				`${user.name} opened a new ticket. Issue: ${ticket.type}`
			);
		...
		close(target, room, user) {
			...
			const helpRoom = /** @type {ChatRoom?} */ (Rooms.get(
				`help-${ticket.userid}`
			));
			if (helpRoom) {
				const ticketGame = /** @type {HelpTicket} */ (helpRoom.game);
		...
		ban(target, room, user) {
			...
			if (
				!targetUser &&
				!Punishments.search(toID(this.targetUsername)).length &&
				!ticket &&
				!ticketBan
			) {
			...
			if (targetUser) {
				...
				if (ticketBan && ticketBan.expires > Date.now())
					return this.privateModAction(
						`(${name} would be ticket banned by ${user.name} but was already ticket banned.)`
					);
			...
			this.addModAction(
				`${name} was ticket banned by ${user.name}.${
					target ? ` (${target})` : ``
				}`
			);
			...
			for (let user of affected) {
				let userid =
					(typeof user !== "string" ? user.getLastId() : toID(user));
		...
		unban(target, room, user) {
			...
			if (ticket.expires <= Date.now()) {
				...
				return this.errorReply(
					`${
						targetUser ? targetUser.name : target
					}'s ticket ban is already expired.`
				);
			}
			...
			for (let t in ticketBans) {
				if (
					toID(ticketBans[t].banned) === toID(ticket.banned) &&
			...
			this.addModAction(
				`${affected.join(", ")} ${Chat.plural(
					affected.length,
					"were",
					"was"
				)} ticket unbanned by ${user.name}.`
			);
		...
		delete(target, room, user) {
			...
			let targetRoom = /** @type {ChatRoom} */ (Rooms.get(
				`help-${ticket.userid}`
			));
			if (targetRoom) {
				// @ts-ignore
