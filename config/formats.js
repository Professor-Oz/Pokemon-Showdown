...
let Formats = [
  ...
  Discussion</a>`,
	...
	},
	{
		...
		onValidateSet(set) {
			...
			if (template.types.includes("Steel"))
				return [
					`${template.species} is a Steel-type, which is banned from Metronome Battle.`
				];
			let bst = 0;
			...
			let types = [
				...new Set(
					target.baseMoveSlots
						.slice(0, 2)
						.map(move => this.dex.getMove(move.id).type)
				)
			];
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			// @ts-ignore
			let oMegaTemplate = this.dex.getTemplate(
				pokemon.template.originalMega
			);
			if (
	...
	},
	{
		...
		onModifyTemplate(template, target, source, effect) {
			...
			for (let statName in pokemon.baseStats) {
				...
				pokemon.baseStats[statName] = this.dex.clampIntRange(
